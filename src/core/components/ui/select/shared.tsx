...
export function flattenItems(props: SelectProps): SelectItemData[] {
  return props.itemGroups?.flatMap((group) => group.items) ?? props.items ?? props.options ?? [];
}

export function itemLabel(item: SelectItemData | undefined, value?: string) {
...
export const SelectNativeTrigger = React.forwardRef<any, SelectTriggerSharedProps>(
  function SelectNativeTrigger(
    { active = false, label: labelProp, onPress, props, value, ...triggerProps },
    ref,
  ) {
...
export const SelectBasicTrigger = React.forwardRef<
  any,
  {
    disabled?: boolean;
    label?: React.ReactNode;
...
>(function SelectBasicTrigger({ disabled, label, props, value, onPress, ...triggerProps }, ref) {
  ...
  const userClassName = cn(
    props.className,
    configuredTriggerProps?.className,
    triggerProps.className,
  );
  const hasCursorOverride = userClassName.split(/\s+/).some((token) => token.startsWith("cursor-"));
  return (
    <Button
      {...configuredTriggerProps}
      {...triggerProps}
      ref={ref}
      className={cn(
        "self-start w-auto justify-between",
        Platform.OS === "web" && "cursor-default",
        props.className,
        configuredTriggerProps?.className,
        triggerProps.className,
      )}
      disabled={resolvedDisabled ?? configuredTriggerProps?.disabled}
      onPress={onPress ?? triggerProps.onPress ?? configuredTriggerProps?.onPress}
      style={(state) => [
        {
          alignSelf: hasFullWidthClass ? "stretch" : "flex-start",
          flexGrow: 0,
        ...
        props.nativeTriggerContainerStyle as any,
...
export function useSelectState(props: SelectProps) {
  ...
  const setValue = React.useCallback(
    (next: string) => {
      if (props.value == null) setUncontrolledValue(next);
...
export function triggerSelectHaptics(props: SelectProps) {
  return useResolvedNativeHaptics(props.nativeHaptics);
}

export function renderNativeItemLabel(item: SelectItemData, selectedValue?: string) {
  const rendered = resolveRenderProp(item.label, {
