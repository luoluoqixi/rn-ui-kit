...
export function ScrollTrack({
  ...
  const {
    thumbBorderRadius = 4,
    thumbColor = defaultColors.thumb,
    thumbHoverColor = defaultColors.thumbHover,
    thumbHeight: fixedThumbHeight,
    minThumbHeight = 28,
    thumbOpacity = 1,
    thumbPressedColor = defaultColors.thumbPressed,
    thumbShadow,
    trackBorderRadius = 4,
    trackColor = defaultColors.track,
    trackHoverColor = defaultColors.trackHover,
    trackOpacity = 1,
    trackPressedColor = defaultColors.trackPressed,
    trackVisible = false,
    trackWidth = 6,
    zIndex = 1000,
  } = styling;
  ...
  const thumbHeight = useMemo(() => {
    if (contentHeight <= containerHeight || containerHeight <= 0) return 0;
    ...
    return clamp(
      fixedThumbHeight ?? Math.max(minimumHeight, dynamicHeight),
      0,
      containerHeight * 0.8,
    );
  ...
  if (containerHeight <= 0 || contentHeight <= containerHeight) return null;
    ...
    <View pointerEvents="box-none" style={[styles.container, { zIndex }]}>
      ...
      {disableGestures ? (
        <View
          ...
          style={[styles.touchArea, { height: containerHeight, width: touchWidth }]}
        >
          {thumb}
        </View>
      ) : (
