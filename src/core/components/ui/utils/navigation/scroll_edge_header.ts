...
const DEFAULT_MANUAL_TOP_THRESHOLD = 1;

type ScrollEvent = NativeSyntheticEvent<NativeScrollEvent>;
type ScrollHandler = NonNullable<ScrollViewProps["onScroll"]>;

type NavigationWithHeaderOptions = {
  setOptions: (options: { headerStyle: StyleProp<ViewStyle>; headerTransparent: boolean }) => void;
};

export type NativeStackScrollEdgeHeaderOptions = {
  /** 页面内容区颜色；Android / Web 位于顶部时使用它，让 Header 与页面自然融为一体。 */
  screenBackgroundColor: string;
  /** 常规 Header 颜色；Android / Web 离开顶部后使用它。 */
...
export type NavigationBarScrollEdgeTrackingOptions = {
  /** Android / Web 顶部状态颜色；默认使用 `useAppBackgroundColors().screen`。 */
  topBackgroundColor?: string;
  /** Android / Web 非顶部状态颜色；默认使用 `useAppBackgroundColors().header`。 */
  scrolledBackgroundColor?: string;
  /** Android / Web 离开顶部的偏移阈值，单位为 pt；默认 1。 */
  topThreshold?: number;
  /**
   * Android / Web 更新 `headerStyle` 时需要保留的额外样式。
...
export type NavigationBarScrollEdgeTrackingProps = {
   ...
   * Android / Web 只在顶部状态发生变化时切换两种实体背景色。
   */
 ...
 * - Android / Web：Header 始终参与正常布局，顶部先使用页面色；滚动后的颜色由页面级
...
/**
 * 合并调用方 onScroll，并只在 Android / Web 的“顶部 / 非顶部”边界发生变化时更新 Header。
...
export function useNavigationBarScrollEdge({
  ...
  const topThreshold = Math.max(
    0,
    navigationBarScrollEdgeOptions?.topThreshold ?? DEFAULT_MANUAL_TOP_THRESHOLD,
  );
  ...
  const applyHeaderState = useCallback(
    (atTop: boolean) => {
      ...
      navigation.setOptions({
        ...
        // Android / Web 只切换两种实体颜色，始终保留 Header 的正常布局占位。
        headerTransparent: false,
      });
    },
    [androidHeaderStyle, enabled, navigation, scrolledBackgroundColor, topBackgroundColor],
  );
