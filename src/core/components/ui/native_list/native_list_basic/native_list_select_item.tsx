...
export function NativeListSelectItem(props: NativeListSelectItemProps) {
  ...
  const [menuOpen, setMenuOpen] = React.useState(Boolean((selectProps as any).defaultOpen));
    ...
    selectedItem?.swatchColor == null ? (
      ...
      <View className="flex-row items-center gap-2">
        <View
          ...
          style={{ backgroundColor: selectedItem.swatchColor }}
        />
        {label}
      </View>
    );
  ...
  const usesNativeDropdownWillChange =
    Platform.OS === "ios" &&
  ...
  const keepsOpenOpacity =
    selectProps.native !== "sheet" && !(selectProps.native === "wheel" && Platform.OS === "ios");
  if (editMode) {
    return (
      <NativeListRow
        {...itemProps}
        ...
        value={
          <View style={{ opacity: NATIVE_LIST_EDIT_VALUE_OPACITY }}>
            {typeof rendered === "string" || typeof rendered === "number" ? (
              <Text style={{ color: triggerColor as string }}>{rendered}</Text>
            ) : (
              triggerLabel
            )}
          </View>
  ...
  return (
    <NativeListRow
      {...itemProps}
      cursorDefault
      labelOpacity={keepsOpenOpacity && menuOpen ? NATIVE_LIST_ITEM_OPEN_OPACITY : 1}
      nativeHaptics={false}
      onPress={() => {
        if (usesIosNativeMenuTrigger && consumeTriggerInteraction()) return;
        if (
          itemProps.disabled ||
      ...
      trailing={
        <View
          collapsable={false}
          onTouchCancel={usesIosNativeMenuTrigger ? cancelTriggerInteraction : undefined}
          onTouchEnd={usesIosNativeMenuTrigger ? finishTriggerInteraction : undefined}
          onTouchStart={usesIosNativeMenuTrigger ? beginTriggerInteraction : undefined}
        >
          <Select
            ref={selectRef}
            {...(selectProps as any)}
