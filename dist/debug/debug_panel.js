...
import { DarkTheme, DefaultTheme, NavigationContainer, StackActions, useNavigation, useRoute, } from "@react-navigation/native";
import { createNativeStackNavigator, } from "@react-navigation/native-stack";
import { useLayoutEffect, useMemo, useState } from "react";
import { Platform, View } from "react-native";
import { useUiTheme } from "../core/components/ui/utils/theme";
import { NativeSheet, NativeSheetStack, getNativeStackScrollEdgeHeaderOptions, isIos26Plus, nativeStackStatusBarOptions, useAppBackgroundColors, useColorSchemeSettings, withNativeBackButton, withNativeStackGestureOptions, RN_UI_KIT_PACKAGE_NAME, RN_UI_KIT_PACKAGE_VERSION, } from "../core";
...
const DEBUG_LARGE_TITLE_OPTIONS = Platform.OS === "ios"
    ? {
...
const DEBUG_REGULAR_TITLE_OPTIONS = Platform.OS === "ios"
    ? {
...
function NativeSheetStackDebugSectionPage(props) {
    // 组件总览的静态页复用详情页的 viewport 约束，不再把外层 ScrollView
    // 注册给 TrueSheet。关闭或切换 Stack screen 时原生 tag 已失效，继续绑定会报 tag 0。
    return _jsx(RnUiKitDebugSectionPage, { ...props, bindToNativeSheet: false });
}
...
function RnUiKitDebugHostPanel({ backButtonLabel, nativeSheetScreenOptions, pageScreenOptions, pages, panelSheetProps, ...props }) {
    ...
    return (_jsx(View, { ...props, style: [{ backgroundColor: appBackgroundColors.screen, flex: 1 }, props.style], children: content }));
}
function RnUiKitDebugHostHomePage({ nativeSheetScreenOptions, onOpenSection, pages: pagesProp, panelSheetProps, ...props }) {
    ...
    return (_jsxs(View, { ...props, style: [{ backgroundColor: appBackgroundColors.screen, flex: 1 }, props.style], children: [_jsx(RnUiKitDebugHomePage, { onRefresh: onRefreshHome, onOpenPanelSheet: () => setPanelSheetOpen(true), onOpenSection: (key) => {
...
function RnUiKitDebugPanelSheet({ nativeSheetScreenOptions, onOpenChange, open, pages, panelSheetProps, ...props }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(NativeSheetStack, { initialRouteName: "index", name: "rn-ui-kit-debug-panel-sheet", onOpenChange: handlePanelOpenChange, open: open, overlayPortalHostName: DEBUG_PANEL_SHEET_OVERLAY_HOST, screenOptions: debugSheetStackScreenOptions, sheetProps: {
                    snapPoints: DEBUG_DEFAULT_SNAP_POINT,
                    ...panelSheetProps,
                }, children: [_jsx(NativeSheetStack.Screen, { name: "index", options: { title: `${RN_UI_KIT_PACKAGE_NAME} - ${RN_UI_KIT_PACKAGE_VERSION}` }, children: () => (_jsx(RnUiKitDebugHomeRoute, { layoutHost: "nativeSheet", onOpenInSheet: (key) => setOpenSectionSheets((current) => new Set(current).add(key)), pages: pages, onOpenSectionsInSheetChange: (enabled) => {
                                setOpenSectionsInSheet(enabled);
                                if (!enabled)
                                    setOpenSectionSheets(new Set());
                            }, onSectionSheetPositionChange: setSectionSheetPosition, openSectionsInSheet: openSectionsInSheet, sectionSheetPosition: sectionSheetPosition })) }), pages.map((definition) => (_jsx(NativeSheetStack.Screen, { name: definition.key, options: { title: definition.label }, children: () => (_jsx(NativeSheetStackDebugSectionPage, { contentTitle: definition.contentTitle, instanceId: `panel-sheet-stack-${definition.key}`, layoutHost: "nativeSheet", pages: pages, sectionKey: definition.key })) }, definition.key))), componentExampleDefinitions.map((definition) => (_jsx(NativeSheetStack.Screen, { name: getComponentExampleRouteName(definition.key), options: {
...
function RnUiKitDebugPanelContent({ initialRouteKey = "components", nativeSheetScreenOptions, pageScreenOptions, pages, panelSheetProps, ...props }) {
    ...
    const [sectionSheetPosition, setSectionSheetPosition] = useState(0);
    const [openSectionSheets, setOpenSectionSheets] = useState(new Set());
    return (_jsxs(View, { ...props, style: [{ backgroundColor: appBackgroundColors.screen, flex: 1 }, props.style], children: [_jsx(NavigationContainer, { theme: navigationTheme, children: _jsxs(Stack.Navigator, { id: "rn-ui-kit-debug-stack", initialRouteName: "index", screenOptions: withNativeBackButton(debugStackScreenOptions), children: [_jsx(Stack.Screen, { name: "index", options: {
                                title: `${RN_UI_KIT_PACKAGE_NAME} - ${RN_UI_KIT_PACKAGE_VERSION}`,
                                ...DEBUG_LARGE_TITLE_OPTIONS,
                            }, children: () => (_jsx(RnUiKitDebugHomeRoute, { onRefresh: onRefreshHome, onOpenInSheet: (key) => setOpenSectionSheets((current) => new Set(current).add(key)), onOpenPanelSheet: () => setPanelSheetOpen(true), pages: pages, onOpenSectionsInSheetChange: (enabled) => {
                                    setOpenSectionsInSheet(enabled);
                                    if (!enabled)
                                        setOpenSectionSheets(new Set());
                                }, onSectionSheetPositionChange: setSectionSheetPosition, openSectionsInSheet: openSectionsInSheet, sectionSheetPosition: sectionSheetPosition })) }), pages.map((definition) => (_jsx(Stack.Screen, { name: definition.key, options: {
                                title: definition.label,
...
function RnUiKitDebugHomeRoute({ layoutHost = "default", onOpenInSheet, pages, onOpenPanelSheet, onOpenSectionsInSheetChange, onSectionSheetPositionChange, openSectionsInSheet, sectionSheetPosition, onRefresh, }) {
    const navigation = useNavigation();
    return (_jsx(RnUiKitDebugHomePage, { onRefresh: onRefresh, layoutHost: layoutHost, onOpenSection: (key) => {
            if (openSectionsInSheet)
