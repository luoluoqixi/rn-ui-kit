...
function triggerNativeDropdownItemHaptics(setting, delay) {
...
function renderItem(item, itemProps, separatorBefore = false, depth = 0) {
    ...
    const hapticsDelay = resolvedItem.nativeHapticsDelay;
    const handleSelect = () => {
    ...
    const isCheckboxItem = Platform.OS === "ios" && (resolvedItem.checkbox || resolvedItem.selected !== undefined);
    const ItemComponent = isCheckboxItem ? Zeego.CheckboxItem : Zeego.Item;
    return (_jsxs(ItemComponent, { "aria-label": accessibilityLabel, destructive: resolvedItem.destructive, disabled: resolvedItem.disabled, onSelect: () => {
            handleSelect();
        }, ...{ separatorBefore }, ...{ selected: resolvedItem.selected }, ...(isCheckboxItem
            ? {
...
function NativeDropdownRoot({ __nativeDetachedAnchor, __menuRef, children, nativeContentProps, defaultOpen, disabled, items, itemProps, itemNativeHaptics, itemNativeHapticsDelay, nativeAnchorAlignment = "center", nativeHaptics, nativeHapticsDelay, nativeShouldWaitForMenuToHideBeforeFiringOnPressMenuItem, nativeSelectedItemBackgroundColor, nativeTrigger, nativeTriggerContainerStyle, nativeTriggerContent, nativeTriggerIcon, nativeTriggerLabelProps, nativeTriggerProps, nativeTriggerFeedbackOpacity, nativeTriggerHoverBackground, onOpenChange, onOpenWillChange, open, trigger, triggerClassName, triggerLabel, triggerProps, ...props }) {
    ...
    const { __unsafeIosProps, style: rootStyle, ...rootProps } = props;
    ...
    const menu = (_jsx(Zeego.Root, { ...rootProps, ...{
            ...
            : __unsafeIosProps == null
                ? {}
                : { __unsafeIosProps }), onOpenChange: handleOpenChange, ...{ onOpenWillChange: handleOpenWillChange }, children: generated ? (_jsxs(_Fragment, { children: [resolvedTrigger != null ? (_jsx(Zeego.Trigger, { asChild: true, ...(isAndroidDetachedTrigger
                        ? { style: { height: anchorSize.height, width: anchorSize.width } }
                        : resolvedTriggerProps), disabled: resolvedDisabled ?? undefined, children: isAndroidDetachedTrigger ? (_jsx(View, { collapsable: false, style: { height: anchorSize.height, width: anchorSize.width } })) : resolvedTrigger })) : null, items != null ? (_jsx(Zeego.Content, { ...nativeContentProps, children: renderItems(items, {
                        nativeHaptics: resolvedItemHaptics,
                        nativeHapticsDelay: resolvedItemHapticsDelay,
                        ...itemProps,
                    }) })) : null, resolvedChildren] })) : (resolvedChildren) }));
    if (isAndroidDetachedTrigger && resolvedTrigger != null && !resolvedDisabled) {
        ...
        return (_jsx(NativeDropdownHapticsContext.Provider, { value: { item: resolvedItemHaptics, itemDelay: resolvedItemHapticsDelay }, children: _jsxs(View, { style: { position: "relative" }, children: [visibleTrigger, _jsx(View, { pointerEvents: "none", style: { bottom: 0, left: 0, position: "absolute", right: 0, top: 0 }, children: menu })] }) }));
    }
    return (_jsx(NativeDropdownHapticsContext.Provider, { value: { item: resolvedItemHaptics, itemDelay: resolvedItemHapticsDelay }, children: resolvedDisabled ? (_jsx(DropdownDisabledTrigger, { children: resolvedTrigger })) : (menu) }));
}
...
function NativeDropdownSubTrigger({ children, disabled, nativeHaptics, nativeHapticsDelay, onPress, onSelect, ...props }) {
    ...
    return (_jsxs(Zeego.SubTrigger, { ...props, disabled: disabled, onSelect: () => {
            triggerNativeDropdownItemHaptics(nativeHaptics ?? contextHaptics.item, nativeHapticsDelay ?? contextHaptics.itemDelay);
            onPress?.();
            onSelect?.();
        }, textValue: label, children: [_jsx(Zeego.ItemTitle, { children: label }), subtitle ? _jsx(Zeego.ItemSubtitle, { children: subtitle }) : null] }));
}
function NativeDropdownItem({ children, nativeHaptics, nativeHapticsDelay, onPress, onSelect, variant, ...props }) {
    const contextHaptics = React.useContext(NativeDropdownHapticsContext);
    const { title: label, subtitle } = resolveItemTitleAndSubtitle(children, props.textValue, props.subtitle);
...
function NativeDropdownCheckboxItem({ checked, children, onCheckedChange, nativeHaptics, nativeHapticsDelay, onValueChange, value, ...props }) {
    const contextHaptics = React.useContext(NativeDropdownHapticsContext);
    const { title: label, subtitle } = resolveItemTitleAndSubtitle(children, props.textValue, props.subtitle);
