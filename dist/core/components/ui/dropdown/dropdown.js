...
const DropdownContentSizeContext = React.createContext("default");
const DropdownHapticsContext = React.createContext({});
function DropdownSubTrigger({ className, inset, children, iconClassName, nativeHaptics, nativeHapticsDelay, onPress, ...props }) {
...
function DropdownSubContent({ className, children, size: sizeProp, style, ...props }) {
    ...
    return (_jsx(NativeOnlyAnimatedView, { entering: FadeIn.reduceMotion(ReduceMotion.System), children: _jsx(DropdownPrimitive.SubContent, { className: cn("bg-popover border-border overflow-hidden rounded-md border p-1 shadow-lg shadow-black/5", Platform.select({
            ...
            style: [Platform.OS === "web" ? semanticColorsToVariables(theme) : null, style], ...props, children: _jsx(DropdownContentSizeContext.Provider, { value: size, children: _jsx(TextClassContext.Provider, { value: cn("text-popover-foreground", menuTextSizeClasses[size]), children: children }) }) }) }));
}
function DropdownContent({ align = "center", className, children, side: sideProp, itemNativeHaptics, itemNativeHapticsDelay, overlayClassName, overlayStyle, portalHost, size: sizeProp, style, ...props }) {
...
function DropdownItem({ className, disabled, inset, nativeHaptics, nativeHapticsDelay, onPress, variant, ...props }) {
    const contextHaptics = React.useContext(DropdownHapticsContext);
    const size = React.useContext(DropdownContentSizeContext);
    return (_jsx(TextClassContext.Provider, { value: cn(cn(menuTextSizeClasses[size], "select-none text-popover-foreground"), !disabled && "group-active:text-popover-foreground", variant === "destructive" && "text-destructive", variant === "destructive" && !disabled && "group-active:text-destructive"), children: _jsx(DropdownPrimitive.Item, { className: cn(cn("group relative flex flex-row items-center gap-2 rounded-sm px-2", menuItemPaddingClasses[size]), !disabled && "active:bg-accent", Platform.select({
...
function DropdownCheckboxItem({ className, children, disabled, nativeHaptics, onCheckedChange, ...props }) {
    const contextHaptics = React.useContext(DropdownHapticsContext);
    const size = React.useContext(DropdownContentSizeContext);
    return (_jsx(TextClassContext.Provider, { value: cn(cn(menuTextSizeClasses[size], "text-popover-foreground select-none"), !disabled && "group-active:text-accent-foreground"), children: _jsxs(DropdownPrimitive.CheckboxItem, { className: cn(cn("group relative flex flex-row items-center gap-2 rounded-sm pl-8 pr-2", menuItemPaddingClasses[size]), !disabled && "active:bg-accent", Platform.select({
                ...
                triggerNativeHaptics(nativeHaptics ?? contextHaptics.item);
            }, children: [_jsx(View, { className: "absolute left-2 flex h-3.5 w-3.5 items-center justify-center", children: _jsx(DropdownPrimitive.ItemIndicator, { children: _jsx(Icon, { as: Check, className: cn(cn("text-foreground shrink-0", menuIconSizeClasses[size]), Platform.select({ web: "pointer-events-none" })) }) }) }), _jsx(_Fragment, { children: children })] }) }));
}
function DropdownRadioItem({ className, children, disabled, nativeHaptics, onPress, ...props }) {
    const contextHaptics = React.useContext(DropdownHapticsContext);
    const size = React.useContext(DropdownContentSizeContext);
    return (_jsx(TextClassContext.Provider, { value: cn(cn(menuTextSizeClasses[size], "text-popover-foreground select-none"), !disabled && "group-active:text-accent-foreground"), children: _jsxs(DropdownPrimitive.RadioItem, { className: cn(cn("group relative flex flex-row items-center gap-2 rounded-sm pl-8 pr-2", menuItemPaddingClasses[size]), !disabled && "active:bg-accent", Platform.select({
                web: "focus:bg-accent focus:text-accent-foreground cursor-default outline-none data-[disabled]:pointer-events-none",
...
function DropdownLabel({ className, inset, ...props }) {
    const size = React.useContext(DropdownContentSizeContext);
    return (_jsx(DropdownPrimitive.Label, { className: cn(cn("text-foreground px-2 font-medium", menuTextSizeClasses[size], menuItemPaddingClasses[size]), inset && "pl-8", className), ...props }));
}
...
function renderDropdownItems(items, itemProps, defaultNativeHaptics, depth = 0) {
    return items.map((item, index) => {
        ...
        const onSelect = (item.onSelect ??
            item.onPress ??
            resolvedItemProps.onPress);
        const itemContent = (_jsxs(_Fragment, { children: [_jsx(Text, { children: label }), resolveRenderProp(item.icon, item), resolveRenderProp(item.indicator, item)] }));
        // 非原生菜单也将 selected 统一呈现为 checkbox，避免平台间丢失勾选状态。
        if (item.selected !== undefined) {
            return (_createElement(DropdownCheckboxItem, { ...resolvedItemProps, "aria-label": item["aria-label"] ?? resolvedItemProps["aria-label"], checked: item.selected, disabled: item.disabled ?? resolvedItemProps.disabled, key: key, nativeHaptics: itemHaptics, onCheckedChange: () => onSelect?.(), textValue: item.textValue ?? resolvedItemProps.textValue }, itemContent));
        }
        return (_createElement(DropdownItem, { ...resolvedItemProps, "aria-label": item["aria-label"] ?? resolvedItemProps["aria-label"], disabled: item.disabled ?? resolvedItemProps.disabled, key: key, nativeHaptics: itemHaptics, onPress: onSelect, textValue: item.textValue ?? resolvedItemProps.textValue, variant: item.destructive === true
                ? "destructive"
                : item.destructive === false
                    ? "default"
                    : resolvedItemProps.variant }, itemContent));
    });
}
function Dropdown({ children, contentProps, contentSize, __nativeDetachedAnchor, __menuRef, defaultOpen, disabled, items, itemProps, itemNativeHaptics, itemNativeHapticsDelay, native = Platform.OS !== "web", nativeAnchorAlignment, nativeHaptics, nativeHapticsDelay, nativeShouldWaitForMenuToHideBeforeFiringOnPressMenuItem, nativeSelectedItemBackgroundColor, nativeTrigger, nativeTriggerContainerStyle, nativeTriggerContent, nativeTriggerIcon, nativeTriggerLabelProps, nativeTriggerProps, nativeTriggerFeedbackOpacity, nativeTriggerHoverBackground, nativeContentProps, open, onOpenChange, onOpenWillChange, trigger, triggerClassName, triggerLabel, triggerProps, ...props }) {
    ...
    return (_jsx(DropdownHapticsContext.Provider, { value: { item: resolvedItemHaptics, itemDelay: resolvedItemHapticsDelay }, children: _jsx(DropdownContentSizeContext.Provider, { value: contentSize ?? "default", children: _jsx(DropdownPrimitiveRoot, { ...props, ...(defaultOpen === undefined ? {} : { defaultOpen }), ...(open === undefined ? {} : { open }), onOpenChange: (open) => {
