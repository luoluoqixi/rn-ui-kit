...
const toggleVariants = cva(cn("group aria-[checked=false]:active:bg-accent/50 aria-[pressed=false]:active:bg-accent/50 data-[state=off]:active:bg-accent/50 flex flex-row items-center justify-center gap-2 rounded-md", Platform.select({
    ...
    variants: {
        variant: {
            ...
            outline: cn("border-input border bg-transparent shadow-sm shadow-black/5", Platform.select({
...
export const ToggleSizeContext = React.createContext("default");
const toggleIconSizes = {
    "default": "size-4",
    "2xs": "size-3",
    "xs": "size-3.5",
    "sm": "size-4",
...
function Toggle({ className, variant, size, nativeHaptics, title, children, onPressedChange, onPressIn, onPressOut, onHoverIn, onHoverOut, ...props }) {
    ...
    return (_jsx(TextClassContext.Provider, { value: cn(toggleTextVariants({ size }), props.pressed && "text-accent-foreground", className), children: _jsx(ToggleSizeContext.Provider, { value: size ?? "default", children: _jsx(TogglePrimitive.Root, { ...props, className: cn(toggleVariants({ variant, size }), props.disabled && "opacity-50", props.pressed && "bg-accent", className), style: interactionStyle, onPressedChange: (pressed) => {
...
function ToggleIcon({ className, ...props }) {
    ...
    return (_jsx(Icon, { className: cn(toggleIconSizes[size], "shrink-0", textClass, className), ...props }));
}
