...
export function ScrollTrack({ alwaysVisible, axis, containerSize, contentSize, disableGestures, hitSlop, insets, onDragEnd, onDragStart, onHoverChange, onPressChange, onPressEnd, onPressStart, onScrollToPosition, scrollPosition, styling, visible, }) {
    ...
    const { thumbBorderRadius = 4, thumbColor = colors.thumb, thumbHoverColor = colors.thumbHover, thumbHeight: fixedThumbLength, minThumbHeight = 28, thumbOpacity = 1, thumbPressedColor = colors.thumbPressed, thumbShadow, trackBorderRadius = 4, trackColor = colors.track, trackHoverColor = colors.trackHover, trackOpacity = 1, trackPressedColor = colors.trackPressed, trackVisible = false, trackWidth = 6, zIndex = 1000, } = styling;
    ...
    const thumbLength = useMemo(() => {
        if (contentSize <= containerSize || containerSize <= 0 || trackLength <= 0)
            return 0;
        const dynamicLength = (trackLength * containerSize) / contentSize;
        const minimumLength = Math.min(minThumbHeight, trackLength * 0.8);
        return clamp(fixedThumbLength ?? Math.max(minimumLength, dynamicLength), 0, trackLength * 0.8);
    ...
    const scrollToTrackPosition = useCallback((touchPosition) => {
        onScrollToPosition(axis, clamp(touchPosition / Math.max(1, trackLength), 0, 1));
    }, [axis, onScrollToPosition, trackLength]);
    const scrollToThumbPosition = useCallback((position) => {
        onScrollToPosition(axis, clamp(position / Math.max(1, maxThumbPosition), 0, 1));
    }, [axis, maxThumbPosition, onScrollToPosition]);
    const getThumbPosition = useCallback(() => clamp((scrollOffset.current / scrollRange) * maxThumbPosition, 0, maxThumbPosition), [maxThumbPosition, scrollRange]);
    const captureDragStart = useCallback((touchPosition) => {
    ...
    const handleDragStart = useCallback((touchPosition) => {
    ...
    const handleDragUpdate = useCallback((touchPosition) => {
        if (draggingThumb.current) {
    ...
    const handlePressEnd = useCallback((touchPosition) => {
        scrollToTrackPosition(touchPosition);
        setPressed(false);
        onPressChange?.(false);
        onPressEnd?.();
    ...
    const panGesture = Gesture.Pan()
        ...
        .onBegin((event) => runOnJS(captureDragStart)(axis === "vertical" ? event.y : event.x))
    ...
    const tapGesture = Gesture.Tap()
        ...
        .onEnd((event, success) => {
        if (success)
            runOnJS(handlePressEnd)(axis === "vertical" ? event.y : event.x);
        else
    ...
    if (containerSize <= 0 || contentSize <= containerSize || trackLength <= 0)
        return null;
