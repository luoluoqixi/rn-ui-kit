...
function SwitchRoot(props) {
    ...
    const nativeSwitchProps = native
        ? {
            ...
            style: platform === "android" && disabled
    ...
    const control = (_jsx(TamaguiSwitch, { native: native, activeStyle: {
        ...
        }, ...rootProps, borderWidth: rootProps.borderWidth ?? 0, checked: checked, cursor: rootProps.cursor ?? "pointer", disabled: disabled, id: controlId, onCheckedChange: handleCheckedChange, nativeProps: nativeSwitchProps, opacity: !native && disabled ? DISABLED_OPACITY : rootProps.opacity, overflow: overflow ?? "hidden", padding: rootProps.padding ?? 0, size: size, children: children ?? _jsx(SwitchThumb, { ...thumbProps, transition: thumbProps?.transition ?? "bouncy" }) }));
    // iOS 原生 UISwitch 作为 flex container 直接子节点时无法正确垂直居中，
    // 套一层 YStack 容器让 flexbox 对齐机制正常工作
