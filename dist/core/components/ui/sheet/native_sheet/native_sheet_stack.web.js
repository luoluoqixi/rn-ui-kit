...
function NativeSheetStackRoot({ children, initialRouteName = "index", name, onOpenChange, open = false, overlayPortalHostName, screenOptions, sheetProps, }) {
    ...
    if (sheetProps == null) {
        return (_jsx(TrueSheetStackHost, { initialRouteName: initialRouteName, name: sheetName, onRequestClose: () => onOpenChange?.(false), onDidDismiss: () => onOpenChange?.(false), overlayPortalHostName: overlayPortalHostName, screenOptions: screenOptions, sheetProps: {
    ...
    const resolvedDismissible = trueSheetProps.dismissible === false || hasOpenSheetAbove
        ? false
