...
function ContextMenuSubTrigger({ className, inset, children, iconClassName, nativeHaptics, onPress, ...props }) {
    const { open } = ContextMenuPrimitive.useSubContext();
    const contextHaptics = React.useContext(ContextMenuHapticsContext);
    const disabled = props.disabled === true;
    const icon = Platform.OS === "web" ? ChevronRight : open ? ChevronUp : ChevronDown;
    return (_jsx(TextClassContext.Provider, { value: cn("text-sm select-none", !disabled && "group-active:text-accent-foreground", open && "text-accent-foreground"), children: _jsxs(ContextMenuPrimitive.SubTrigger, { className: cn("group flex flex-row items-center justify-between rounded-sm px-2 py-2 sm:py-1.5", !disabled && "active:bg-accent", Platform.select({
...
function ContextMenuContent({ className, overlayClassName, overlayStyle, portalHost, style, itemNativeHaptics, side: sideProp, children, ...props }) {
...
function ContextMenuCheckboxItem({ className, children, disabled, nativeHaptics, onCheckedChange, ...props }) {
    const contextHaptics = React.useContext(ContextMenuHapticsContext);
    return (_jsx(TextClassContext.Provider, { value: cn("text-sm text-popover-foreground select-none", !disabled && "group-active:text-accent-foreground"), children: _jsxs(ContextMenuPrimitive.CheckboxItem, { className: cn("group relative flex flex-row items-center gap-2 rounded-sm py-2 pl-8 pr-2 sm:py-1.5", !disabled && "active:bg-accent", Platform.select({
                ...
                triggerNativeHaptics(nativeHaptics ?? contextHaptics.item);
            }, children: [_jsx(View, { className: "absolute left-2 flex h-3.5 w-3.5 items-center justify-center", children: _jsx(ContextMenuPrimitive.ItemIndicator, { children: _jsx(Icon, { as: Check, className: cn("text-foreground size-4", Platform.select({ web: "pointer-events-none" })) }) }) }), _jsx(_Fragment, { children: children })] }) }));
}
function ContextMenuRadioItem({ className, children, disabled, nativeHaptics, onPress, ...props }) {
    const contextHaptics = React.useContext(ContextMenuHapticsContext);
    return (_jsx(TextClassContext.Provider, { value: cn("text-sm text-popover-foreground select-none", !disabled && "group-active:text-accent-foreground"), children: _jsxs(ContextMenuPrimitive.RadioItem, { className: cn("group relative flex flex-row items-center gap-2 rounded-sm py-2 pl-8 pr-2 sm:py-1.5", !disabled && "active:bg-accent", Platform.select({
                web: "focus:bg-accent focus:text-accent-foreground cursor-default outline-none data-[disabled]:pointer-events-none",
...
function ContextMenuLabel({ className, inset, ...props }) {
    return (_jsx(ContextMenuPrimitive.Label, { className: cn("text-foreground px-2 py-2 text-sm font-medium sm:py-1.5", inset && "pl-8", className), ...props }));
}
...
function ContextMenuItemTitle({ className, ...props }) {
    return _jsx(Text, { className: cn("text-sm text-popover-foreground", className), ...props });
}
function ContextMenuItemSubtitle({ className, ...props }) {
    return _jsx(Text, { className: cn("text-muted-foreground text-xs", className), ...props });
}
...
function ContextMenu({ children, items, itemProps, itemNativeHaptics, native = Platform.OS !== "web", nativeHaptics, nativeShouldWaitForMenuToHideBeforeFiringOnPressMenuItem, __menuRef, __unsafeIosProps, onOpenChange, onOpenWillChange, trigger, triggerProps, ...props }) {
    ...
    return (_jsx(ContextMenuHapticsContext.Provider, { value: { item: resolvedItemHaptics }, children: _jsx(ContextMenuPrimitiveRoot, { ...props, onOpenChange: (open) => {
