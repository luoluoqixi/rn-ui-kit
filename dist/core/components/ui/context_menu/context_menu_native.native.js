...
function renderItems(items, itemProps, defaultNativeHaptics, depth = 0) {
    if (Platform.OS === "ios") {
        const groups = splitMenuItemsBySeparators(items);
        if (groups.length > 1) {
            return groups.map((group, index) => (_jsx(Zeego.Group, { children: renderItems(group, itemProps, defaultNativeHaptics, depth + 1) }, `${depth}:group:${index}`)));
        }
        items = groups[0] ?? [];
    }
...
function ContextMenu({ children, items, itemProps, itemNativeHaptics, nativeHaptics, onOpenChange, onOpenWillChange, trigger, triggerProps, nativeShouldWaitForMenuToHideBeforeFiringOnPressMenuItem, __menuRef, __unsafeIosProps, ...props }) {
    ...
    return (_jsx(NativeContextMenuHapticsContext.Provider, { value: { item: itemHaptics }, children: _jsx(Zeego.Root, { ...props, ...{ __menuRef }, ...(Platform.OS === "ios"
                ? {
                    __unsafeIosProps: {
                        ...__unsafeIosProps,
                ...
                onOpenChange?.(open);
            }, ...{ onOpenWillChange }, children: generated ? (_jsxs(_Fragment, { children: [trigger != null ? (_jsx(Zeego.Trigger, { ...{ action: "longPress", ...triggerProps }, children: resolveRenderProp(trigger, { native: true, open: false }) })) : null, items != null ? (_jsx(Zeego.Content, { children: renderItems(items, { nativeHaptics: itemHaptics, ...itemProps }, itemHaptics) })) : null, children] })) : (children) }) }));
}
