...
import { NativePressRow, NativeHostedTrailingControl, supportsNativeTextRow, styles, } from "../native_list_native.ios";
...
export function NativeListDropdownItem({ dropdownProps, ...itemProps }) {
    ...
    const inheritedHaptics = useResolvedNativeListHaptics(dropdownProps.nativeHaptics ?? itemProps.nativeHaptics);
    ...
    return (_jsx(NativePressRow, { ...itemProps, disabled: disabled ?? undefined, labelOpacity: fadeTitleOnOpen && menuOpen ? NATIVE_LIST_ITEM_OPEN_OPACITY : 1, nativeHaptics: false, onPress: () => {
            if (menuRef.current == null || menuOpen || presentingMenuRef.current)
                return;
            ...
            menuRef.current.presentMenu();
        }, trailingControl: _jsx(NativeHostedTrailingControl, { disableInEditMode: true, children: _jsx(Menu, { ...dropdownProps, nativeHaptics: inheritedHaptics, nativeTrigger: true, nativeTriggerContainerStyle: [styles.selectInlineTrigger, { paddingHorizontal: 0 }], nativeTriggerIcon: "chevrons-up-down", nativeTriggerLabelProps: nativeTriggerLabelProps, nativeTriggerFeedbackOpacity: {
                ...
                }, triggerLabel: menuValue, onOpenChange: dropdownProps.onOpenChange, onOpenWillChange: (nextOpen) => {
