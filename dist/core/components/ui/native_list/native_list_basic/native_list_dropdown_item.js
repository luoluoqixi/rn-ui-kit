...
export function NativeListDropdownItem(props) {
    ...
    const [uncontrolledWillOpen, setUncontrolledWillOpen] = React.useState(Boolean(dropdownProps.defaultOpen));
    ...
    const disabled = Boolean(itemProps.disabled || dropdownProps.disabled || dropdownProps.triggerProps?.disabled);
    const label = itemProps.value ?? itemProps.title ?? "更多";
    const triggerColor = itemProps.valueColor ?? theme[NATIVE_LIST_TRAILING_TRIGGER_COLOR_TOKEN];
    ...
    if (editMode) {
        return (_jsx(NativeListRow, { ...itemProps, disabled: disabled, valueColor: triggerColor, value: label, valueOpacity: NATIVE_LIST_EDIT_VALUE_OPACITY }));
    }
    ...
    return (_jsx(NativeListRow, { ...itemProps, cursorDefault: true, disabled: disabled, labelOpacity: props.fadeTitleOnOpen !== false && menuOpen ? NATIVE_LIST_ITEM_OPEN_OPACITY : 1, nativeHaptics: false, onPress: () => {
            if (Platform.OS === "ios" && consumeTriggerInteraction())
                return;
            if (disabled || menuOpen || presentingMenuRef.current)
                return;
            itemProps.onPress?.();
            ...
            presentMenuFromRow();
        }, pressedOpacity: NATIVE_LIST_ITEM_PRESS_OPACITY, suppressPressBackground: true, value: undefined, trailing: _jsx(View, { collapsable: false, onTouchCancel: Platform.OS === "ios" ? cancelTriggerInteraction : undefined, onTouchEnd: Platform.OS === "ios" ? finishTriggerInteraction : undefined, onTouchStart: Platform.OS === "ios" ? beginTriggerInteraction : undefined, style: { alignItems: "center", alignSelf: "stretch", justifyContent: "center" }, children: _jsx(Dropdown, { ...dropdownProps, __menuRef: menuRef, ...(dropdownProps.open === undefined ? {} : { open: dropdownProps.open }), nativeHaptics: inheritedHaptics, nativeTrigger: true, nativeTriggerIcon: "chevrons-up-down", nativeTriggerContainerStyle: { paddingHorizontal: 0 }, nativeTriggerLabelProps: nativeTriggerLabelProps, nativeTriggerProps: {
                    ...dropdownProps.nativeTriggerProps,
                    fontWeight: nativeTriggerFontWeight ??
                ...
                }, triggerLabel: label, triggerProps: { ...dropdownProps.triggerProps, disabled } }) }) }));
}
