...
export function NativeListSelectItem({ selectProps, ...itemProps }) {
    ...
    return (_jsx(NativeListRow, { ...itemProps, cursorDefault: true, labelOpacity: menuOpen ? NATIVE_LIST_ITEM_OPEN_OPACITY : 1, nativeHaptics: false, pressedOpacity: NATIVE_LIST_ITEM_PRESS_OPACITY, suppressPressBackground: true, onPress: () => {
            itemProps.onPress?.();
            openSelect();
        }, value: undefined, trailing: _jsx(Select, { ref: selectRef, ...selectProps, nativeTrigger: true, nativeTriggerIcon: selectProps.nativeTriggerIcon ?? "chevrons-up-down", nativeHaptics: inheritedHaptics, nativeTriggerHoverBackground: false, nativeTriggerLabel: selectProps.nativeTriggerLabel, nativeTriggerLabelProps: {
...
export function NativeListDropdownItem({ dropdownProps, ...itemProps }) {
    ...
    return (_jsx(NativeListRow, { ...itemProps, cursorDefault: true, nativeHaptics: false, labelOpacity: fadeTitleOnOpen && menuOpen ? NATIVE_LIST_ITEM_OPEN_OPACITY : 1, onPress: () => {
            ...
            menuRef.current?.presentMenu();
                ...
                open: dropdownProps.open ?? menuOpen, __menuRef: menuRef, nativeHaptics: inheritedHaptics, nativeTrigger: true, nativeTriggerIcon: "chevrons-up-down", nativeTriggerLabelProps: nativeTriggerLabelProps, nativeTriggerProps: {
...
export function NativeListRoot({ children, contextMenuProps, contentContainerStyle, defaultSelectedIds, disabledStyle, nativeHaptics, editMode, editModeIcon, editModeSelectedIcon, editModeSelectedSfSymbol, editModeSfSymbol, iosListStyle: _iosListStyle, listStyle = NATIVE_LIST_BASIC_DEFAULT_STYLE, listStyleOptions, onRefresh, onSelectedIdsChange, refreshColor, refreshEnabledInEditMode: _refreshEnabledInEditMode, scrollable = true, selectedIds, style, ...scrollViewProps }) {
    void _iosListStyle;
    void _refreshEnabledInEditMode;
    const backgrounds = useAppBackgroundColors();
    const theme = useUiTheme();
    const { borderColor, borderRadius, dividerColor, dividerPaddingLeft, dividerPaddingRight, dividerWidth, rowBackgroundColor, sectionShadow = NATIVE_LIST_BASIC_STYLE_DEFAULTS.sectionShadow, showBorder, showDivider = NATIVE_LIST_BASIC_STYLE_DEFAULTS.showDivider, } = listStyleOptions ?? {};
    const resolvedShowBorder = showBorder ?? listStyle === "rounded";
    const [refreshing, setRefreshing] = useState(false);
    const content = (_jsx(NativeListBasicStyleContext.Provider, { value: listStyle, children: _jsx(NativeListBasicBorderRadiusContext.Provider, { value: borderRadius, children: _jsx(NativeListBasicShowBorderContext.Provider, { value: resolvedShowBorder, children: _jsx(NativeListBasicBorderColorContext.Provider, { value: borderColor, children: _jsx(NativeListBasicDividerColorContext.Provider, { value: dividerColor, children: _jsx(NativeListBasicRowBackgroundColorContext.Provider, { value: rowBackgroundColor, children: _jsx(NativeListBasicSectionShadowContext.Provider, { value: sectionShadow, children: _jsx(NativeListBasicShowDividerContext.Provider, { value: showDivider, children: _jsx(NativeListBasicDividerWidthContext.Provider, { value: dividerWidth ?? NATIVE_LIST_BASIC_STYLE_DEFAULTS.dividerWidth, children: _jsx(NativeListBasicDividerPaddingContext.Provider, { value: dividerPaddingLeft ?? NATIVE_LIST_BASIC_STYLE_DEFAULTS.dividerPaddingLeft, children: _jsx(NativeListBasicDividerRightPaddingContext.Provider, { value: dividerPaddingRight ??
    ...
    if (!scrollable) {
        return (_jsx(View, { style: [styles.root, { backgroundColor: backgrounds.screen }, style], children: content }));
    }
    return (_jsx(ScrollView, { ...scrollViewProps, contentContainerStyle: [styles.content, contentContainerStyle], refreshControl: onRefresh == null ? undefined : (_jsx(RefreshControl, { colors: [refreshColor ?? theme.primary], enabled: !editMode, onRefresh: () => {
                setRefreshing(true);
                Promise.resolve(onRefresh()).finally(() => setRefreshing(false));
            }, refreshing: refreshing, tintColor: refreshColor ?? theme.primary })), style: [styles.root, { backgroundColor: backgrounds.screen }, style], children: content }));
}
