...
export function NativeListCustomItem({ backgroundColor, children, contextMenuProps, disabled, hoverBackgroundColor, nativeHaptics, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, pressBackgroundColor, selectionId, selectionDisabled, }) {
    const restoresIos15TopCorners = useContext(Ios15FirstVisibleRowContext);
    const resolvedContextMenuProps = useResolvedNativeListContextMenu(contextMenuProps);
    const editRow = useNativeListEditRow({
        disabled,
    ...
    if (editRow.editMode) {
        return (_jsx(NativeRowContainer, { ...rowPaddingProps, disabled: disabled, nativeSelectionId: editRow.nativeSelection ? editRow.selectionId : undefined, nativeScrollId: nativeScrollId, onPress: editRow.onPress, children: _jsx(NativeHostedCustomRow, { disableInteractions: true, children: children }) }));
    }
    if (onPress == null) {
        const rowModifiers = [
            ROW_INSETS,
        ...
        const customRow = (_jsx(VStack, { modifiers: rowModifiers, children: _jsx(NativeHostedCustomRow, { disabled: Boolean(disabled), children: children }) }));
        if (swiftUIContextMenuProps != null) {
    ...
    if (restoresIos15TopCorners) {
        const button = (_jsx(SwiftButton, { modifiers: [disabledModifier(disabled ?? false)], onPress: () => {
                ...
                ], children: _jsx(NativeHostedCustomRow, { disabled: Boolean(disabled), children: children }) }) }));
        return swiftUIContextMenuProps != null ? (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: swiftUIContextMenuProps, children: button })) : (button);
    }
    const button = (_jsx(SwiftButton, { modifiers: [
            disabledModifier(disabled ?? false),
