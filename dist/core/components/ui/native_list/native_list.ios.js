...
import { ContextMenu as SwiftContextMenu, Divider as SwiftDivider, HStack, Host, Image, List, RNHostView, Spacer, Button as SwiftButton, Text as SwiftText, Section as SwiftUISection, Toggle as SwiftToggle, VStack, ZStack, } from "@expo/ui/swift-ui";
import { background, buttonStyle, contentMargins, contentShape, disabled as disabledModifier, environment, font, foregroundStyle, frame, ios15ListRowTopRoundedBackground, layoutPriority, lineLimit, listRowBackground, listRowInsets, listRowSeparator, listSectionSpacing, listStyle, multilineTextAlignment, onLongPressGesture, opacity, padding, refreshable, scrollContentBackground, scrollDisabled, shapes, tag, tint, toggleStyle, viewID, } from "@expo/ui/swift-ui/modifiers";
...
import { NativeListEditModeProvider, useNativeListEditIcons, useNativeListEditMode, useNativeListEditRow, } from "./edit_mode";
...
function supportsSwiftUIContextMenuItems(items) {
    return items.every((item) => item["aria-label"] == null &&
        item.icon == null &&
        item.indicator == null &&
        item.subtitle == null &&
...
function canUseSwiftUIContextMenu(contextMenuProps) {
    if (contextMenuProps == null) {
        return false;
    }
    const supportedRootProps = new Set(["itemProps", "items", "nativeHaptics", "triggerProps"]);
    const hasUnsupportedRootProps = Object.entries(contextMenuProps).some(([key, value]) => value !== undefined && !supportedRootProps.has(key));
    const hasUnsupportedItemProps = Object.entries(contextMenuProps.itemProps ?? {}).some(([key, value]) => value !== undefined && key !== "destructive" && key !== "disabled");
    const hasUnsupportedTriggerProps = Object.entries(contextMenuProps.triggerProps ?? {}).some(([key, value]) => value !== undefined && key !== "disabled");
    return (!hasUnsupportedRootProps &&
...
function NativeSwiftUIContextMenuItems({ itemProps, items, }) {
    return items.map((item) => {
        ...
        if (item.subMenu?.length) {
            return (_jsxs(SwiftContextMenu, { children: [_jsx(SwiftContextMenu.Trigger, { children: _jsx(SwiftButton, { label: label, modifiers: [disabledModifier(disabled)], role: destructive ? "destructive" : "default", systemImage: item.selected ? "checkmark" : undefined }) }), _jsx(SwiftContextMenu.Items, { children: _jsx(NativeSwiftUIContextMenuItems, { itemProps: itemProps, items: item.subMenu }) })] }, item.value));
        }
        return (_jsx(SwiftButton, { label: label, modifiers: [disabledModifier(disabled)], onPress: () => {
                const handler = item.onSelect ?? item.onPress;
                handler?.();
            }, role: destructive ? "destructive" : "default", systemImage: item.selected ? "checkmark" : undefined }, item.value));
    });
}
...
function resolveNativeListEditIndicatorBorderColor(theme) {
    return (toSwiftUIHexColor(theme.gray8?.val) ??
...
function NativeRowLabel({ subtitle, subtitleColor, subtitleFontSize, title, titleAlign, expand = false, titleColor, titleFontSize, titleLineLimit, layoutPriorityValue = 1, preserveLeadingAnchor = false, }) {
...
function NativeRowContainer({ children, contextMenuProps, disabled, editingSelected, nativeSelectionId, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, btnStyle, btnTint, rowAlignment = "center", rowMinHeight, }) {
    const theme = useTheme();
    ...
    const selectedBackgroundColor = toSwiftUIHexColor(theme.color5?.val ?? theme.backgroundPress?.val ?? theme.background?.val) ??
        theme.color5?.val;
    ...
    const contextMenuAnchor = hostedContextMenuProps != null ? (_jsx(NativeHostedContextMenu, { contextMenuProps: hostedContextMenuProps, menuRef: contextMenuRef })) : null;
    const baseModifiers = [
        ROW_INSETS,
    ...
    if (onPress != null) {
        const button = (_jsx(SwiftButton, { modifiers: [
                ...
                ...(hostedContextMenuProps != null
                    ...
                    : []),
            ], onPress: onPress, children: hostedContextMenuProps != null ? (_jsxs(ZStack, { alignment: "center", children: [contextMenuAnchor, buttonContent] })) : (buttonContent) }));
        return usesSwiftUIContextMenu && contextMenuProps != null ? (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: contextMenuProps, children: button })) : (button);
    }
    const rowModifiers = [
        ...baseModifiers,
        disabledModifier(disabled ?? false),
    ...
    if (usesSwiftUIContextMenu && contextMenuProps != null) {
        return (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: contextMenuProps, children: _jsx(HStack, { alignment: rowAlignment, modifiers: rowModifiers, spacing: 12, children: children }) }));
    }
    if (hostedContextMenuProps != null) {
        return (_jsxs(ZStack, { alignment: "leading", modifiers: [
                ...rowModifiers,
                onLongPressGesture(() => contextMenuRef.current?.presentMenu()),
            ], children: [contextMenuAnchor, _jsx(HStack, { alignment: rowAlignment, modifiers: [frame({ maxWidth: 99999 })], spacing: 12, children: children })] }));
    }
    return (_jsx(HStack, { alignment: rowAlignment, modifiers: rowModifiers, spacing: 12, children: children }));
}
function NativeEditingIndicator({ selected }) {
    const theme = useTheme();
    const { editModeIcon, editModeSelectedIcon, editModeSelectedSfSymbol, editModeSfSymbol } = useNativeListEditIcons();
    ...
    const accentColor = toSwiftUIHexColor(theme.accent10?.val ?? theme.color10?.val ?? theme.color.val) ??
        theme.accent10?.val ??
        theme.color10?.val ??
        theme.color.val;
    const borderColor = resolveNativeListEditIndicatorBorderColor(theme);
    if (configuredSfSymbol != null) {
        return (_jsx(Image, { color: selected ? accentColor : borderColor, size: 24, systemName: configuredSfSymbol }));
    }
    if (customIcon != null) {
        return _jsx(NativeHostedEditingIcon, { children: customIcon });
    }
    return (_jsx(Image, { color: selected ? accentColor : borderColor, size: 24, systemName: selected ? "checkmark.circle.fill" : "circle" }));
}
...
function NativeHostedEditingIcon({ children }) {
    return (_jsx(RNHostView, { matchContents: true, children: _jsx(View, { collapsable: false, style: styles.hostedEditingIcon, children: children }) }));
}
function NativeHostedContextMenu({ contextMenuProps, menuRef, }) {
    return (_jsx(RNHostView, { matchContents: true, children: _jsx(View, { collapsable: false, pointerEvents: "none", style: styles.contextMenuAnchor, children: _jsx(ContextMenu, { ...contextMenuProps, trigger: _jsx(View, { collapsable: false, style: styles.contextMenuAnchor }), ...{ __menuRef: menuRef } }) }) }));
}
...
function NativeHostedTrailingControl({ children, disableInEditMode = false, }) {
    ...
    return (_jsx(RNHostView, { matchContents: true, children: _jsx(View, { collapsable: false, pointerEvents: interactionsDisabled ? "none" : "auto", style: styles.trailingHostedContent, children: children }) }));
}
...
function NativePressRow({ chevron = false, chevronColor, contextMenuProps, disabled, icon, iconColor, iconSize, iconSlotWidth, sfSymbol, nativeHaptics, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, selected = false, selectionId, subtitle, subtitleColor, subtitleFontSize, trailing, title, titleAlign, titleColor, titleFontSize, titleLineLimit, trailingControl, overlayTrailingControlOnValueSymbol = false, preserveValueWidth = false, value, valueColor, valueFontSize, valueSfSymbol, btnStyle, btnTint, preserveLeadingAnchor = false, rowAlignment = "center", rowMinHeight, }) {
    ...
    return (_jsxs(NativeRowContainer, { contextMenuProps: editRow.editMode || resolvedContextMenuProps?.triggerProps?.disabled
            ? undefined
            : resolvedContextMenuProps, disabled: disabled, editingSelected: editRow.nativeSelection ? false : editRow.editingSelected, nativeSelectionId: editRow.nativeSelection ? editRow.selectionId : undefined, onPress: handlePress, btnStyle: btnStyle, btnTint: btnTint, nativeScrollId: nativeScrollId, paddingBottom: paddingBottom, paddingHorizontal: paddingHorizontal, paddingLeft: paddingLeft, paddingRight: paddingRight, paddingTop: paddingTop, paddingVertical: paddingVertical, rowAlignment: rowAlignment, rowMinHeight: rowMinHeight, children: [editRow.editMode && !editRow.nativeSelection ? (_jsx(NativeEditingIndicator, { selected: editRow.editingSelected }, "edit-mode-indicator")) : null, sfSymbol != null ? (_jsx(ZStack, { alignment: "center", modifiers: [frame({ width: resolvedIconSlotWidth, alignment: "center" })], children: _jsx(Image, { color: resolvedIconColor, size: resolvedIconSize, systemName: sfSymbol }) }, "leading-icon")) : icon != null ? (_jsx(NativeHostedIcon, { children: icon }, "leading-icon")) : null, _jsx(NativeRowLabel, { subtitle: subtitleText ?? undefined, subtitleColor: subtitleColor, subtitleFontSize: subtitleFontSize, title: titleText ?? undefined, titleAlign: titleAlign, expand: titleAlign != null, titleColor: titleColor ?? btnTint, titleFontSize: titleFontSize, titleLineLimit: titleLineLimit, layoutPriorityValue: preserveValueWidth ? 0 : 1, preserveLeadingAnchor: preserveLeadingAnchor }, "row-label"), showTrailingSpacer ? _jsx(Spacer, { minLength: 12 }, "trailing-spacer") : null, valueText != null ? (_jsx(SwiftText, { modifiers: [
...
function NativeListRoot({ automaticallyAdjustsScrollIndicatorInsets, backgroundColor, children, contextMenuProps, contentInsetAdjustmentBehavior, contentMarginBottom, contentMarginTop, defaultSelectedIds, editMode, editModeIcon, editModeSelectedIcon, editModeSelectedSfSymbol, editModeSfSymbol, fixesIOS26NestedScrollIndicatorSafeArea, initialScrollTarget, iosEditModeVariant = "native", native = true, nestedScrollEnabled, navigationBarScrollEdgeOptions, onRefresh, onSelectedIdsChange, scrollIndicatorInsets, style, scrollable = true, selectedIds, tracksNavigationBarScrollEdge, webAutoRestoreScroll: _webAutoRestoreScroll, ...fallbackProps }) {
    ...
    return (_jsx(NativeListEditModeProvider, { defaultSelectedIds: defaultSelectedIds, editMode: editMode, editModeIcon: iosEditModeVariant === "custom" ? editModeIcon : undefined, editModeSelectedIcon: iosEditModeVariant === "custom" ? editModeSelectedIcon : undefined, editModeSelectedSfSymbol: iosEditModeVariant === "custom" ? editModeSelectedSfSymbol : undefined, editModeSfSymbol: iosEditModeVariant === "custom" ? editModeSfSymbol : undefined, nativeSelectionEnabled: iosEditModeVariant === "native", onSelectedIdsChange: handleSelectedIdsChange, selectedIds: resolvedSelectedIds, children: _jsx(NativeListContext.Provider, { value: { native: true }, children: _jsx(Host, { style: [styles.nativeRoot, style], children: _jsx(List
...
function NativeListSection({ children, contextMenuProps, footer, trailing, title, titleColor, titleFontSize, }) {
    ...
    if (!nativeListEnabled) {
        return (_jsx(FallbackSection, { contextMenuProps: contextMenuProps, footer: footer, trailing: trailing, title: title, titleColor: titleColor, titleFontSize: titleFontSize, children: children }));
    }
    ...
    const header = trailing != null || usesIos15HeaderRow ? (_jsxs(HStack, { alignment: "center", modifiers: [
            frame({
                maxWidth: 99999,
                alignment: "leading",
            }),
            ...(usesIos15HeaderRow
                ...
                : []),
                    ...
                    ...(resolvedSectionTitleColor != null
                        ...
                        : []),
                ], children: stringTitle })) : title != null ? (_jsx(NativeHostedContent, { children: title })) : null, trailing != null ? _jsx(Spacer, { minLength: 0 }) : null, trailing != null ? _jsx(NativeTrailingContent, { children: trailing }) : null] })) : stringTitle != null && (resolvedSectionTitleColor != null || titleFontSize != null) ? (_jsx(SwiftText, { modifiers: [
            font({ size: titleFontSize ?? DEFAULT_SECTION_TITLE_FONT_SIZE, weight: "regular" }),
            ...(resolvedSectionTitleColor != null
                ? [foregroundStyle(resolvedSectionTitleColor)]
                : []),
        ], children: stringTitle })) : title != null && stringTitle == null ? (_jsx(NativeHostedContent, { children: title })) : undefined;
    const footerView = stringFooter != null ? (_jsx(SwiftText, { modifiers: subtitleModifiers(), children: stringFooter })) : footer != null ? (_jsx(NativeHostedContent, { children: footer })) : undefined;
...
export function NativeListSwitchItem({ switchProps, ...itemProps }) {
    ...
    return (_jsx(NativePressRow, { ...itemProps, nativeHaptics: itemProps.nativeHaptics ?? !editMode, disabled: disabled, onPress: () => {
            ...
            ], onIsOnChange: handleCheckedChange }), value: undefined }));
}
export function NativeListSelectItem({ selectProps, ...itemProps }) {
    ...
    const usesStableNativeValue = nativeTriggerText != null &&
    ...
    return (_jsx(NativePressRow, { ...itemProps, disabled: disabled, nativeHaptics: resolvedHaptics, onPress: () => {
            pickerRef.current?.open();
                ...
                ], nativeTriggerContent: selectProps.nativeTriggerContent, nativeTriggerIcon: usesStableNativeValue ? "none" : (selectProps.nativeTriggerIcon ?? "chevrons-up-down"), nativeTriggerLabel: usesStableNativeValue ? "" : nativeTriggerLabel, nativeTriggerLabelProps: {
                ...
                nativeTriggerPressedOpacity: resolvedPickerMode === "wheel" ? false : undefined, onOpenChange: selectProps.onOpenChange, onValueChange: selectProps.onValueChange, placeholder: selectProps.placeholder, resolvedNativeHaptics: resolvedHaptics, value: selectedValue ?? null }) }), value: usesStableNativeValue ? nativeTriggerText : undefined, valueColor: usesStableNativeValue ? (itemProps.valueColor ?? nativeValueColor) : itemProps.valueColor, valueSfSymbol: usesStableNativeValue && selectProps.nativeTriggerIcon !== "none"
            ? "chevron.up.chevron.down"
            : undefined, overlayTrailingControlOnValueSymbol: usesStableNativeValue && selectProps.nativeTriggerIcon !== "none", preserveValueWidth: usesStableNativeValue }));
}
...
export function NativeListMenuItem({ menuProps, ...itemProps }) {
    ...
    return (_jsx(NativePressRow, { ...itemProps, disabled: disabled, nativeHaptics: false, onPress: () => menuRef.current?.presentMenu(), trailingControl: _jsx(NativeHostedTrailingControl, { disableInEditMode: true, children: _jsx(Menu, { ...menuProps, nativeHaptics: menuProps.nativeHaptics ?? itemProps.nativeHaptics ?? false, nativeTrigger: true, nativeTriggerContainerStyle: [
                    usesStableNativeValue ? styles.invisibleTrailingTrigger : styles.selectInlineTrigger,
                    disabled ? styles.disabledContent : null,
                ], nativeTriggerIcon: usesStableNativeValue ? "none" : "chevrons-up-down", nativeTriggerLabel: usesStableNativeValue ? "" : menuValue, nativeTriggerLabelProps: {
                    color: itemProps.valueColor ?? "$color10",
                    fontSize: itemProps.valueFontSize ?? "$4",
                    numberOfLines: 1,
                    opacity: 1,
                ...
                __menuRef: menuRef }) }), value: usesStableNativeValue ? menuValueText : menuValue, valueColor: usesStableNativeValue ? (itemProps.valueColor ?? nativeValueColor) : itemProps.valueColor, valueSfSymbol: usesStableNativeValue ? "chevron.up.chevron.down" : undefined, overlayTrailingControlOnValueSymbol: usesStableNativeValue, preserveValueWidth: usesStableNativeValue }));
}
export function NativeListCustomItem({ backgroundColor, children, contextMenuProps, disabled, hoverBackgroundColor, nativeHaptics, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, pressBackgroundColor, selectionId, }) {
    ...
    if (editRow.editMode) {
        return (_jsxs(NativeRowContainer, { ...rowPaddingProps, btnStyle: "plain", disabled: disabled, editingSelected: editRow.nativeSelection ? false : editRow.editingSelected, nativeSelectionId: editRow.nativeSelection ? editRow.selectionId : undefined, nativeScrollId: nativeScrollId, onPress: editRow.onPress, children: [!editRow.nativeSelection ? (_jsx(NativeEditingIndicator, { selected: editRow.editingSelected })) : null, _jsx(NativeHostedCustomRow, { disableInteractions: true, children: children })] }));
    }
    if (onPress == null) {
        const rowModifiers = [
            ROW_INSETS,
        ...
        if (usesSwiftUIContextMenu && activeContextMenuProps != null) {
            return (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: activeContextMenuProps, children: customRow }));
        }
        if (hostedContextMenuProps != null) {
            return (_jsxs(ZStack, { alignment: "leading", modifiers: [
                    ...rowModifiers,
                    onLongPressGesture(() => contextMenuRef.current?.presentMenu()),
                ], children: [contextMenuAnchor, _jsx(NativeHostedCustomRow, { children: children })] }));
        }
        return customRow;
    }
    const resolvedHaptics = useResolvedNativeHaptics(nativeHaptics);
    if (restoresIos15TopCorners) {
        const button = (_jsx(SwiftButton, { modifiers: [
                ...
                onPress();
                triggerNativeHaptics(resolvedHaptics);
            }, children: hostedContextMenuProps != null ? (_jsxs(ZStack, { alignment: "center", modifiers: [
                    ROW_INSETS,
                    padding(resolveRowPadding(rowPaddingProps)),
                    frame({ maxWidth: 99999, alignment: "leading" }),
                    ios15ListRowTopRoundedBackground(12, {
                ...
                ], children: [contextMenuAnchor, _jsx(NativeHostedCustomRow, { children: children })] })) : (_jsx(VStack, { modifiers: [
                    ROW_INSETS,
                    padding(resolveRowPadding(rowPaddingProps)),
                    frame({ maxWidth: 99999, alignment: "leading" }),
                    ios15ListRowTopRoundedBackground(12, {
        ...
        return usesSwiftUIContextMenu && activeContextMenuProps != null ? (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: activeContextMenuProps, children: button })) : (button);
    }
    const button = (_jsx(SwiftButton, { modifiers: [
            disabledModifier(disabled ?? false),
            ROW_INSETS,
            ...
            onPress();
            triggerNativeHaptics(resolvedHaptics);
        }, children: hostedContextMenuProps != null ? (_jsxs(ZStack, { alignment: "center", children: [contextMenuAnchor, _jsx(NativeHostedCustomRow, { children: children })] })) : (_jsx(NativeHostedCustomRow, { children: children })) }));
    return usesSwiftUIContextMenu && activeContextMenuProps != null ? (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: activeContextMenuProps, children: button })) : (button);
}
