...
import { NativeListContextMenuProvider, resolveNativeListContextMenu, useResolvedNativeListContextMenu, } from "./context_menu";
...
const FallbackListScrollCaptureContext = createContext(null);
...
function FallbackRowContainer({ backgroundColor, children, contextMenuProps, disabled, editingSelected, hoverBackgroundColor, nativeHaptics, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, pressResetToken, pressBackgroundColor, }) {
    ...
    const contextMenuAnchor = activeNativeContextMenuProps != null ? (_jsx(FallbackNativeContextMenuAnchor, { contextMenuProps: activeNativeContextMenuProps, menuRef: contextMenuRef })) : null;
    ...
    const row = (_jsx(Pressable, { disabled: disabled, onHoverIn: () => setHovered(true), onHoverOut: () => setHovered(false), onPressIn: usesIosSwitchPressFallback ? () => setPressed(true) : undefined, onLongPress: activeNativeContextMenuProps != null
            ? () => contextMenuRef.current?.presentMenu()
            : undefined, onPress: onPress != null
            ? () => {
                captureListScrollPosition?.();
                onPress();
                triggerNativeHaptics(resolvedHaptics);
            }
            : undefined, onPressOut: usesIosSwitchPressFallback ? () => setPressed(false) : undefined, style: styles.pressable, children: ({ pressed: pressablePressed }) => (_jsx(View, { style: [
                styles.rowContainer,
                resolvedRowPadding,
...
function NativeListRow({ backgroundColor, chevron = false, chevronColor, contextMenuProps, disabled, hoverBackgroundColor, icon, iconAfter, iconSlotWidth, nativeHaptics, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, pressResetToken, pressBackgroundColor, selectionId, selected = false, subtitle, subtitleColor, subtitleFontSize, trailing, title, titleAlign, titleColor, titleFontSize, value, valueColor, valueFontSize, }) {
...
function createFallbackRowEntry(child, key, sectionKey, inheritedContextMenuProps) {
    const contextMenuProps = resolveNativeListContextMenu(getFallbackContextMenuProps(child), inheritedContextMenuProps);
...
function appendSectionEntries(entries, sectionProps, sectionKey, inheritedContextMenuProps) {
    ...
    const hasSectionContent = sectionProps.title != null ||
    ...
    sectionChildren.forEach((child, index) => {
        entries.push(createFallbackRowEntry(child, `${sectionKey}-row-${getNodeKey(child, String(index))}`, sectionKey, sectionContextMenuProps));
    });
...
function createFallbackListEntries(children, contextMenuProps) {
    const entries = [];
    Children.toArray(children).forEach((child, index) => {
        if (isNativeListSectionElement(child)) {
            appendSectionEntries(entries, child.props, getNodeKey(child, `section-${index}`), contextMenuProps);
            return;
        }
        entries.push(createFallbackRowEntry(child, `direct-row-${getNodeKey(child, String(index))}`, `direct-${index}`, contextMenuProps));
    });
    return entries;
}
function renderFallbackListEntry({ item, }) {
    switch (item.type) {
        ...
        case "row":
            return (_jsx(NativeListEditRowIdProvider, { selectionId: item.nativeScrollId ?? item.key, children: _jsx(FallbackListContextMenuRow, { contextMenuProps: item.contextMenuProps, children: _jsx(FallbackListRowFrame, { children: item.renderRow() }) }) }));
        case "sectionFooter":
...
export function NativeListSwitchItem({ switchProps, ...itemProps }) {
    ...
    const editMode = useNativeListEditMode();
    return (_jsx(NativeListRow, { ...itemProps, disabled: disabled, nativeHaptics: itemProps.nativeHaptics ?? !editMode, onPress: () => switchProps.onCheckedChange?.(!checked), pressResetToken: isIos ? pressResetToken : undefined, iconAfter: _jsx(View, { style: styles.trailingControl, children: _jsx(Switch, { ...switchProps, native: true, onPress: (event) => {
...
export function NativeListInputItem({ inputProps, ...itemProps }) {
    ...
    if (hasLeadingLabel) {
        return (_jsx(NativeListRow, { ...itemProps, disabled: disabled, iconAfter: _jsx(View, { style: styles.inputTrailing, children: resolvedInput }) }));
    }
...
export function NativeListCustomItem({ backgroundColor, children, contextMenuProps, disabled, hoverBackgroundColor, nativeHaptics, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, pressBackgroundColor, selectionId, }) {
    const editRow = useNativeListEditRow({ disabled, nativeScrollId, onPress, selectionId });
    return (_jsx(FallbackRowContainer, { backgroundColor: backgroundColor, contextMenuProps: contextMenuProps, disabled: disabled, editingSelected: editRow.editingSelected, hoverBackgroundColor: hoverBackgroundColor, nativeHaptics: nativeHaptics, onPress: editRow.onPress, paddingBottom: paddingBottom, paddingHorizontal: paddingHorizontal, paddingLeft: paddingLeft, paddingRight: paddingRight, paddingTop: paddingTop, paddingVertical: paddingVertical, pressBackgroundColor: pressBackgroundColor, children: _jsxs(View, { pointerEvents: editRow.editMode ? "none" : "auto", style: editRow.editMode ? styles.editingCustomRowContent : styles.customRowContent, children: [editRow.editMode ? _jsx(FallbackEditingIndicator, { selected: editRow.editingSelected }) : null, _jsx(View, { style: styles.customRowContent, children: children })] }) }));
}
export function NativeListSection({ children, contextMenuProps, footer, trailing, title, titleColor, titleFontSize, }) {
    const entries = createFallbackListEntries(_jsx(NativeListSection, { footer: footer, contextMenuProps: contextMenuProps, trailing: trailing, title: title, titleColor: titleColor, titleFontSize: titleFontSize, children: children }));
    return _jsx(View, { style: styles.staticSection, children: renderStaticEntries(entries) });
}
export function NativeListRoot({ backgroundColor, children, contextMenuProps, contentContainerStyle, contentMarginBottom, contentMarginTop, defaultSelectedIds, editMode, editModeIcon, editModeSelectedIcon, editModeSelectedSfSymbol, editModeSfSymbol, fixesIOS26NestedScrollIndicatorSafeArea: _fixesIOS26NestedScrollIndicatorSafeArea, initialScrollTarget, iosEditModeVariant: _iosEditModeVariant, native: _native, navigationBarScrollEdgeOptions, onRefresh, onSelectedIdsChange, scrollable = true, selectedIds, style, tracksNavigationBarScrollEdge = false, ...rest }) {
    void _native;
    void _fixesIOS26NestedScrollIndicatorSafeArea;
    void _iosEditModeVariant;
    const { alwaysBounceVertical, automaticallyAdjustsScrollIndicatorInsets, contentInset, contentInsetAdjustmentBehavior, contentOffset, keyboardShouldPersistTaps, maintainVisibleContentPosition: _maintainVisibleContentPosition, nestedScrollEnabled, onLayout, onScroll, scrollEventThrottle, scrollIndicatorInsets, showsVerticalScrollIndicator, webAutoRestoreScroll = true, ...scrollViewProps } = rest;
    ...
    const entries = useMemo(() => createFallbackListEntries(children, contextMenuProps), [children, contextMenuProps]);
    ...
    const handleFlashListLayout = useCallback((event) => {
        onLayout?.(event);
