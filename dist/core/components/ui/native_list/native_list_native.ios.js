...
import { background, buttonStyle, contentMargins, contentShape, createModifier, disabled as disabledModifier, font, foregroundStyle, frame, ios15ListRowTopRoundedBackground, ios15ListRowSeparatorHidden, layoutPriority, lineLimit, listRowBackground, listRowInsets, listSectionSpacing, listStyle, multilineTextAlignment, opacity, padding, scrollContentBackground, scrollDisabled, shapes, tag, tint, viewID, } from "@luoluoqixi/expo-ui-55/swift-ui/modifiers";
import { Children, Fragment, createContext, useCallback, useContext, useEffect, useMemo, useRef, useState, } from "react";
...
const nativeAllowsHitTesting = (enabled) => createModifier("allowsHitTesting", { enabled });
...
const NativeListWithNavigationSelection = List;
...
const NativeNavigationSelectionButton = SwiftButton;
const NativeListNavigationSelectionContext = createContext({
...
export function NativeSwiftUIContextMenu({ children, contextMenuProps, disabled = false, }) {
    return (_jsxs(SwiftContextMenu, { enabled: !disabled, children: [_jsx(SwiftContextMenu.Trigger, { children: children }), _jsx(SwiftContextMenu.Items, { children: _jsx(NativeSwiftUIContextMenuItems, { itemProps: contextMenuProps.itemProps, items: contextMenuProps.items ?? [] }) })] }));
}
 ...
 */
function NativeListIos15BottomSpacer({ length }) {
    if (length <= 0) {
        return null;
    }
    return (_jsx(VStack, { modifiers: [
            ROW_INSETS,
            frame({ height: length, maxWidth: 99999 }),
            listRowBackground("clear"),
            ios15ListRowSeparatorHidden(),
        ], children: _jsx(SwiftText, { children: "" }) }));
}
...
export function NativeRowContainer({ children, contextMenuProps, contextMenuDisabled, disabled, disabledStyle, ios15FirstRowTopInset = IOS15_FIRST_ROW_TOP_INSET, nativeSelectionActive, nativeSelectionId, nativeNavigationSelectionId, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, btnStyle, btnTint, rowAlignment = "center", rowMinHeight, }) {
    const theme = useTheme();
    ...
    const swiftUIContextMenuProps = (!disabled || contextMenuDisabled) && hasSwiftUIContextMenu(contextMenuProps)
        ? contextMenuProps
        : undefined;
    const baseModifiers = [
        ROW_INSETS,
    ...
    if (onPress != null) {
        const button = (_jsx(NativeNavigationSelectionButton, { modifiers: [
            ...
            ], listSelectionId: nativeNavigationSelectionId, onPress: onPress, children: buttonContent }));
        return swiftUIContextMenuProps != null ? (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: swiftUIContextMenuProps, disabled: contextMenuDisabled, children: button })) : (button);
    }
    const rowModifiers = [
        ...baseModifiers,
        disabledModifier(disabled ?? false),
    ...
    if (swiftUIContextMenuProps != null) {
        return (_jsx(NativeSwiftUIContextMenu, { contextMenuProps: swiftUIContextMenuProps, disabled: contextMenuDisabled, children: _jsx(HStack, { alignment: rowAlignment, modifiers: rowModifiers, spacing: 12, children: children }) }));
    }
...
export function NativePressRow({ chevron = false, chevronColor, contextMenuProps, disabled, disabledStyle, icon, iconColor, iconSize, iconSlotWidth, sfSymbol, nativeHaptics, nativeScrollId, onPress, paddingBottom, paddingHorizontal, paddingLeft, paddingRight, paddingTop, paddingVertical, selected = false, selectionId, selectionDisabled, subtitle, subtitleColor, subtitleFontSize, trailing, title, titleAlign, titleColor, titleFontSize, titleLineLimit, trailingControl, overlayTrailingControlOnValueSymbol = false, preserveValueWidth = false, labelOpacity = 1, value, valueColor, valueFontSize, valueSfSymbol, btnStyle, btnTint, preserveLeadingAnchor = false, rowAlignment = "center", rowMinHeight, ios15RowType = "text", iosNavigationSelection = false, iosNavigationSelectionAutoClear = true, iosNavigationSelectionAutoClearDelay, }) {
    ...
    const isBarePlainTextRow = subtitleText == null &&
        !chevron &&
        icon == null &&
        sfSymbol == null &&
        !selected &&
        ...
        valueText == null &&
        valueSfSymbol == null;
    const ios15FirstRowTopInset = subtitleText != null
        ...
        ? () => {
            if (iosNavigationSelection && !editRow.editMode) {
                const controls = isIos15()
                    ? undefined
                    : navigationSelection.selectNavigationItem(editRow.selectionId, {
    ...
    const nativeSelectionId = iosNavigationSelection &&
        !editRow.editMode &&
        !isIos15() &&
        !disabled &&
        handlePress != null
        ...
        : isIos15() && !selectionDisabled
            ...
            : editRow.nativeSelection
                ? editRow.selectionId
                : undefined;
    const contextMenuUnavailable = Boolean(disabled || editRow.editMode || resolvedContextMenuProps?.triggerProps?.disabled);
    ...
    return (_jsxs(NativeRowContainer, { contextMenuProps: preservesIos15ContextMenu || !contextMenuUnavailable ? resolvedContextMenuProps : undefined, contextMenuDisabled: preservesIos15ContextMenu && contextMenuUnavailable, disabled: disabled, disabledStyle: disabledStyle, ios15FirstRowTopInset: ios15FirstRowTopInset, nativeSelectionActive: editRow.nativeSelection, nativeSelectionId: nativeSelectionId, nativeNavigationSelectionId: iosNavigationSelection &&
            !editRow.editMode &&
            !isIos15() &&
            !disabled &&
            handlePress != null
            ? editRow.selectionId
            : undefined, onPress: handlePress, btnStyle: resolvedBtnStyle, btnTint: btnTint, nativeScrollId: nativeScrollId, paddingBottom: paddingBottom, paddingHorizontal: paddingHorizontal, paddingLeft: paddingLeft, paddingRight: paddingRight, paddingTop: paddingTop, paddingVertical: paddingVertical, rowAlignment: rowAlignment, rowMinHeight: rowMinHeight, children: [sfSymbol != null ? (_jsx(ZStack, { alignment: "center", modifiers: [frame({ width: resolvedIconSlotWidth, alignment: "center" })], children: _jsx(Image, { color: resolvedIconColor, size: resolvedIconSize, systemName: sfSymbol }) }, "leading-icon")) : icon != null ? (_jsx(NativeHostedIcon, { children: icon }, "leading-icon")) : null, _jsx(NativeRowLabel, { subtitle: subtitleText ?? undefined, subtitleColor: subtitleColor, subtitleFontSize: subtitleFontSize, title: titleText ?? undefined, titleAlign: titleAlign, expand: titleAlign != null, titleColor: titleColor ?? btnTint, titleFontSize: titleFontSize, titleLineLimit: titleLineLimit, layoutPriorityValue: preserveValueWidth ? 0 : 1, opacityValue: labelOpacity, preserveLeadingAnchor: preserveLeadingAnchor }, "row-label"), showTrailingSpacer ? _jsx(Spacer, { minLength: 12 }, "trailing-spacer") : null, valueText != null ? (_jsx(SwiftText, { modifiers: [
                    ...valueModifiers(valueFontSize),
                    foregroundStyle(resolvedValueColor),
                    ...(preserveValueWidth ? [layoutPriority(2)] : []),
                ], children: valueText }, "row-value")) : null, valueSfSymbol != null ? (_jsxs(ZStack, { alignment: "center", modifiers: preserveValueWidth ? [layoutPriority(2)] : undefined, children: [_jsx(Image, { color: resolvedValueColor, size: 13, systemName: valueSfSymbol }), overlayTrailingControlOnValueSymbol && trailingControl != null ? (_jsx(Fragment, { children: trailingControl }, "trailing-control-overlay")) : null] }, "row-value-symbol")) : null, selected && (!editRow.editMode || isIos15()) ? (_jsx(Image, { color: accentColor, modifiers: isIos15() ? [opacity(editRow.editMode ? 0 : 1)] : undefined, size: 18, systemName: "checkmark" }, "selected-checkmark")) : null, trailing != null ? (_jsx(NativeTrailingContent, { children: trailing }, "custom-trailing")) : null, trailingControl != null && !overlayTrailingControlOnValueSymbol ? (_jsx(Fragment, { children: trailingControl }, "trailing-control")) : null, !editRow.editMode && chevron ? (_jsx(Image, { color: resolvedChevronColor, size: 13, systemName: "chevron.right" }, "chevron")) : null] }));
}
function NativeListRoot({ automaticallyAdjustsScrollIndicatorInsets, backgroundColor, children, contextMenuProps, disabledStyle, contentInsetAdjustmentBehavior, contentMarginBottom, contentMarginTop, defaultSelectedIds, dismissKeyboardOnTap = false, editMode, editModeIcon, editModeSelectedIcon, editModeSelectedSfSymbol, editModeSfSymbol, fixesIOS26NestedScrollIndicatorSafeArea, initialScrollTarget, iosListStyle = "insetGrouped", nestedScrollEnabled, navigationBarScrollEdgeOptions, onRefresh, onSelectedIdsChange, nativeHaptics, iosPressFeedback = "immediate", refreshColor: _refreshColor, refreshEnabledInEditMode = false, scrollIndicatorInsets, style, scrollable = true, selectedIds, tracksNavigationBarScrollEdge, webAutoRestoreScroll: _webAutoRestoreScroll, }) {
    ...
    const [navigationSelectionId, setNavigationSelectionId] = useState(undefined);
    const navigationSelectionIdRef = useRef(undefined);
    const navigationSelectionTimeoutRef = useRef(undefined);
    ...
    const clearNavigationSelection = useCallback((selectionId) => {
        if (selectionId != null && navigationSelectionIdRef.current !== selectionId)
            return;
    ...
    const selectNavigationItem = useCallback((selectionId, { autoClear, autoClearDelay = DEFAULT_NAVIGATION_SELECTION_AUTO_CLEAR_DELAY, }) => {
    ...
    useEffect(() => {
        if (navigation == null || isIos15())
            return;
        const removeTransitionListener = navigation.addListener("transitionStart", (event) => {
    ...
    return (_jsx(NativeListNavigationSelectionContext.Provider, { value: navigationSelectionContext, children: _jsx(NativeListEditModeProvider, { defaultSelectedIds: defaultSelectedIds, editMode: editMode, nativeSelectionEnabled: true, onSelectedIdsChange: handleSelectedIdsChange, selectedIds: resolvedSelectedIds, children: _jsx(Host, { style: [styles.nativeRoot, style], children: _jsxs(NativeListWithNavigationSelection
                ...
                , { 
                    ...
                    compensatesForViewportClipping: compensatesForTrueSheetViewportClipping, correctsNestedScrollIndicatorFrame: isIos26Plus() && fixesIOS26NestedScrollIndicatorSafeArea === true, delaysContentTouches: !usesImmediatePressFeedback, dismissKeyboardOnTap: dismissKeyboardOnTap, initialScrollAnchor: "center", initialScrollTarget: initialScrollTarget, nativeEditMode: usesNativeEditMode ? "active" : "inactive", nativeEditTint: nativeEditTint, clearsNavigationSelectionOnViewWillAppear: !usesNativeEditMode && !isIos15(), onNavigationSelectionCleared: () => clearNavigationSelection(), onSelectionChange: usesNativeEditMode ? handleSelectedIdsChange : undefined, selection: usesNativeEditMode
                        ? [...resolvedSelectedIds]
                        : navigationSelectionId == null
                            ? []
                            : [navigationSelectionId], onRefresh: handleNativeRefresh, 
                    ...
                    ], children: [_jsx(NativeListContextMenuProvider, { contextMenuProps: contextMenuProps, disabledStyle: disabledStyle, children: _jsx(NativeListHapticsProvider, { nativeHaptics: nativeHaptics, children: children }) }), _jsx(NativeListIos15BottomSpacer, { length: ios15BottomSpacerLength })] }) }) }) }));
}
