...
function ToggleGroup({ className, variant, size, nativeHaptics, children, items, ...props }) {
    ...
    return (_jsx(ToggleGroupPrimitive.Root, { className: cn("flex flex-row items-center rounded-md shadow-none", Platform.select({ web: "w-fit" }), variant === "outline" && "shadow-sm shadow-black/5", className), ...props, children: _jsx(ToggleGroupContext.Provider, { value: { nativeHaptics: resolvedNativeHaptics, variant, size }, children: children ??
...
function ToggleGroupItem({ className, children, variant, size, isFirst, isLast, nativeHaptics, title, onPress, ...props }) {
    ...
    return (_jsx(TextClassContext.Provider, { value: cn("text-sm text-foreground font-medium", ToggleGroupPrimitive.utils.getIsSelected(value, props.value)
            ? "text-accent-foreground"
            : Platform.select({ web: "group-hover:text-muted-foreground" })), children: _jsx(ToggleGroupPrimitive.Item, { ...props, className: cn(toggleVariants({
