...
export function NativeSlider(props) {
    const { value, onValueChange, onValueChangeFinished, min, max, step: stepProp, style, colors: colorsProp, nativeHaptics, nativeHapticsInterval, } = props;
    ...
    const resolvedSteps = safeStep != null ? Math.max(0, Math.round((safeMax - safeMin) / safeStep) - 1) : 0;
    ...
    return (_jsx(Host, { style: [{ height: 48, justifyContent: "center", width: "100%" }, style], children: _jsx(ExpoSlider, { value: currentValue, onValueChange: handleValueChange, onValueChangeFinished: () => onValueChangeFinished?.([latestValueRef.current]), min: safeMin, max: safeMax, steps: resolvedSteps, colors: resolvedColors }) }));
}
