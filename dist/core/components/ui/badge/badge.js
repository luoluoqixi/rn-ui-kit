...
const badgeVariants = cva(cn("border-border group shrink-0 flex-row items-center justify-center overflow-hidden rounded-full border", Platform.select({
...
function Badge({ className, icon, iconClassName, iconProps, label, labelClassName, labelProps, size = "default", variant, asChild, children, ...props }) {
    const Component = asChild ? Slot : View;
    const renderedLabel = resolveRenderProp(label, { icon, size, variant });
    const content = children ?? (_jsxs(_Fragment, { children: [icon ? (_jsx(Icon, { as: icon, ...iconProps, className: cn({
                    "size-2.5": size === "2xs",
                    ...
                    "size-5": size === "xl",
                    "size-6": size === "2xl",
                }, iconClassName, iconProps?.className) })) : null, renderedLabel != null
                ? normalizeBadgeChildren(renderedLabel, cn(labelClassName, labelProps?.className), labelProps)
                : null] }));
    return (_jsx(TextClassContext.Provider, { value: badgeTextVariants({ size, variant }), children: _jsx(Component, { className: cn(badgeVariants({ size, variant }), className), ...props, children: content }) }));
}
