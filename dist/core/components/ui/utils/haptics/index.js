...
export function triggerNativeHaptics(setting, options) {
    ...
    const milliseconds = Math.max(0, (options?.delay?.milliseconds ?? 0) + (options?.delay?.seconds ?? 0) * 1000);
...
function scheduleNativeHapticsFrames(trigger, frames) {
    if (frames <= 0) {
        trigger();
        return;
    }
...
function triggerNativeHapticsNow(setting, androidType) {
    const level = setting === true ? "light" : setting;
    if (os() === "android") {
        Haptics.performAndroidHapticsAsync(androidType ?? ANDROID_HAPTICS_TYPE_MAP[level]).catch((err) => {
