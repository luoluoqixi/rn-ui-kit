...
function AccordionTrigger({ className, children, headerProps, onPressIn, onPressOut, ...props }) {
    ...
    return (_jsx(TextClassContext.Provider, { value: cn("text-left text-sm font-medium", Platform.select({ web: isPressed && "underline" }), Platform.select({ web: "group-hover:underline" })), children: _jsx(AccordionPrimitive.Header, { ...headerProps, children: _jsx(AccordionPrimitive.Trigger, { ...props, asChild: Platform.OS !== "web", onPressIn: (event) => {
                    setIsPressed(true);
                    onPressIn?.(event);
                }, onPressOut: (event) => {
                    setIsPressed(false);
