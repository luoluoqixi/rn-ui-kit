...
function TooltipRootComponent({ children, content, contentProps, triggerProps, ...props }) {
    if (content === undefined) {
        return _jsx(TooltipRoot, { ...props, children: children });
    }
    ...
    return (_jsxs(TooltipRoot, { ...props, children: [triggerElement, _jsx(TooltipContent, { ...contentProps, children: renderedContent })] }));
}
function TooltipContent({ className, children, sideOffset = 4, portalHost, side = "top", style, ...props }) {
    ...
    return (_jsx(TooltipPrimitive.Portal, { hostName: resolvedPortalHost, children: _jsx(OverlayPortalWindow, { portalHost: resolvedPortalHost, children: _jsx(TooltipPrimitive.Overlay, { style: Platform.select({ native: StyleSheet.absoluteFill }), asChild: Platform.OS !== "web", children: _jsx(NativeOnlyAnimatedView, { entering: side === "top"
                        ...
                        }).reduceMotion(ReduceMotion.System), exiting: FadeOut.reduceMotion(ReduceMotion.System), as: "Pressable", children: _jsx(TextClassContext.Provider, { value: "text-xs text-primary-foreground", children: _jsx(TooltipPrimitive.Content, { sideOffset: sideOffset, style: contentStyle, className: cn("bg-primary z-50 rounded-md px-3 py-2 sm:py-1.5", Platform.select({
