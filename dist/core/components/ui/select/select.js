...
function SelectItemSwatch({ color, leading = false, leadingGap = 8, }) {
    const swatch = (_jsx(View, { accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants", style: {
    ...
    if (leading) {
        return (_jsx(View, { accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants", style: { height: SELECT_ITEM_SWATCH_SIZE, width: SELECT_ITEM_SWATCH_SIZE + leadingGap }, children: swatch }));
    }
...
function renderSelectTriggerLabel(label, isPlaceholder, swatchColor) {
    const renderedLabel = renderSelectWebMenuTriggerLabel(label, isPlaceholder);
    if (swatchColor == null) {
        return renderedLabel;
    }
    return (_jsxs(XStack, { flex: 1, items: "center", children: [_jsx(SelectItemSwatch, { color: swatchColor, leading: true, leadingGap: 12 }), renderedLabel] }));
}
function renderSelectNativeTriggerLabel(label, swatchColor, labelProps) {
    if (swatchColor == null) {
        return label;
    }
    const renderedLabel = typeof label === "string" || typeof label === "number" ? (_jsx(SizableText, { color: "$color", fontSize: getFontSize("$4"), opacity: typeof labelProps?.opacity === "number" ? labelProps.opacity : 0.58, ...labelProps, children: label })) : (label);
    return (_jsxs(XStack, { items: "center", children: [_jsx(SelectItemSwatch, { color: swatchColor, leading: true }), renderedLabel] }));
}
...
function IosNativeSheetSelectList({ initialScrollTarget, itemGroups, itemLabel, nativeHaptics, value, }) {
    ...
    return (_jsx(NativeList, { native: true, scrollable: true, contentMarginBottom: DEFAULT_IOS_NATIVE_LIST_CONTENT_MARGIN_BOTTOM, contentMarginTop: DEFAULT_IOS_NATIVE_LIST_CONTENT_MARGIN_TOP, initialScrollTarget: initialScrollTarget, style: { flex: 1, minHeight: 0, width: "100%" }, children: itemGroups.map((group, groupIndex) => {
            ...
            return (_jsx(NativeListSection, { title: groupLabel, children: group.items.map((item) => {
                    ...
                    return (_jsx(NativeListItem, { chevron: false, disabled: disabled, iconColor: item.swatchColor, iconSize: item.swatchColor != null ? 14 : undefined, iconSlotWidth: item.swatchColor != null ? 24 : undefined, nativeHaptics: nativeHaptics, nativeScrollId: item.value, onPress: disabled
                            ? undefined
                            : () => {
                                itemParentContext.onChange(item.value);
                                itemParentContext.setSelectedItem(item.label);
                                itemParentContext.setOpen(false);
                            }, selected: item.value === value, sfSymbol: item.swatchColor != null ? "circle.fill" : undefined, title: item.label }, item.value));
                }) }, group.key));
        }) }));
}
...
const SelectRoot = forwardRef(({ "aria-label": ariaLabel, children, contentProps, disabled, isDisabled, itemIndicatorProps, itemGroups, itemProps, itemTextProps, items, itemLabel, itemLabelProps, nativeHaptics, nativeDropdownAlign, nativeDropdownAnchorWidth, nativeDropdownEdgeOffset, nativePickerMode, onOpenChange, onValueChange, options, placeholder, placement, touchSheetMaxHeight, triggerProps, viewportProps, webMenuArrow, native, nativeTrigger, nativeTriggerContainerStyle, nativeTriggerContent, nativeTriggerIcon, nativeTriggerLabelProps, ...props }, ref) => {
    ...
    const getItemByValue = (value) => resolvedItems.find((item) => item.value === value);
    const getItemLabelByValue = (value) => getItemByValue(value)?.label ?? null;
    ...
    const triggerValueLabel = selectedValue == null || selectedValue === "" || props.renderValue == null
        ? triggerLabel
        : props.renderValue(selectedValue);
    const nativeTriggerLabelValue = triggerValueLabel;
    const nativeTriggerLabel = renderSelectNativeTriggerLabel(nativeTriggerLabelValue, selectedSwatchColor, nativeTriggerLabelProps);
    const renderTriggerValue = (nextValue) => renderSelectTriggerLabel(props.renderValue ? props.renderValue(nextValue) : getItemLabelByValue(nextValue), false, getItemByValue(nextValue)?.swatchColor);
    ...
    const renderItem = (item) => (_createElement(SelectItem, { ...(shouldUseTouchSheetLayout
            ...
            : DEFAULT_SELECT_ITEM_CONTENT_STYLE, children: [item.startContent, shouldUseTouchSheetLayout && item.swatchColor != null ? (_jsx(SelectItemSwatch, { color: item.swatchColor, leading: true })) : null, _jsx(SelectItemText, { fontSize: shouldUseTouchSheetLayout ? "$4" : undefined, lineHeight: shouldUseTouchSheetLayout ? 22 : undefined, ...itemTextProps, children: item.label }), item.description, item.swatchColor != null ? (_jsxs(_Fragment, { children: [item.endContent, _jsx(YStack, { items: "center", justify: "center", ml: "auto", width: SELECT_ITEM_INDICATOR_SLOT_WIDTH, children: _jsx(SelectItemIndicator, { ...itemIndicatorProps, children: itemIndicatorProps?.children ??
                                (shouldUseTouchSheetLayout ? _jsx(Check, { size: 22 }) : undefined) }) }), !shouldUseTouchSheetLayout ? _jsx(SelectItemSwatch, { color: item.swatchColor }) : null] })) : (_jsxs(_Fragment, { children: [item.endContent, _jsx(SelectItemIndicator, { marginLeft: "auto", ...itemIndicatorProps, children: itemIndicatorProps?.children ??
    ...
    const renderWebMenuItem = (item) => {
        ...
        return (_createElement(Menu.RadioItem, { ...itemProps, "aria-label": resolveAriaLabel(item["aria-label"] ?? itemProps?.["aria-label"], item.label), disabled: itemDisabled, key: item.value, textValue: item.label, value: item.value, ...{ "data-rn-ui-kit-select-menu-item-value": item.value } },
            ...
            item.swatchColor != null ? (_jsxs(_Fragment, { children: [item.endContent, _jsx(YStack, { items: "center", justify: "center", ml: "auto", width: SELECT_ITEM_INDICATOR_SLOT_WIDTH, children: _jsx(Menu.ItemIndicator, { ...itemIndicatorProps, children: itemIndicatorProps?.children ??
                                (isSelected ? _jsx(Check, { color: "$color10", size: 12 }) : null) }) }), _jsx(SelectItemSwatch, { color: item.swatchColor })] })) : (_jsxs(_Fragment, { children: [item.endContent, _jsx(Menu.ItemIndicator, { marginLeft: "auto", ...itemIndicatorProps, children: itemIndicatorProps?.children ??
    ...
    return (_jsxs(_Fragment, { children: [shouldRenderWebMenuSelect ? (resolvedItems.length === 0 ? null : (_jsxs(Menu, { modal: true, nativeHaptics: resolvedNativeHaptics, onOpenChange: handleWebMenuOpenChange, open: resolvedWebMenuOpen, offset: 8, placement: placement, children: [_jsx(Menu.Trigger, { asChild: true, disabled: selectDisabled, children: webMenuTrigger }), _jsxs(Menu.Portal, { zIndex: resolvedWebMenuContentZIndex, children: [resolvedWebMenuOpen ? (_jsx(YStack, { "aria-hidden": true, onClick: handleWebMenuOverlayPress, onMouseDown: handleWebMenuOverlayPress, onPointerDown: handleWebMenuOverlayPress, style: {
                    ...
                    ? false
                    : (props.open ?? (imperativeOpen ? true : undefined)), native: selectBehavior.tamaguiNative, onOpenChange: handleTamaguiOpenChange, onValueChange: handleTamaguiValueChange, renderValue: renderTriggerValue, children: children ??
