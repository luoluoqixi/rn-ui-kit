...
function renderSelectDisplay(label, swatchColor) {
...
function SelectTrigger({ className, children, size = "default", ...props }) {
    return (_jsxs(SelectPrimitive.Trigger, { className: cn("border-input bg-background flex h-10 flex-row items-center justify-between gap-2 rounded-md border px-3 py-2 shadow-sm shadow-black/5", Platform.select({
        ...
        }), size === "sm" && "h-8 py-1.5", props.disabled && "opacity-50", className), ...props, children: [typeof children === "function" ? children({ pressed: false }) : normalizeText(children), _jsx(Icon, { as: ChevronDown, "aria-hidden": true, className: "text-muted-foreground size-4 shrink-0" })] }));
}
function SelectContent({ className, children, position = "popper", align = "center", side: sideProp, showScrollButtons = true, initialScrollOffset = 0, portalHost, viewportProps, ...props }) {
                ...
                style: Platform.OS === "web"
                    ...
                    : StyleSheet.absoluteFillObject, pointerEvents: Platform.OS === "web" ? "auto" : undefined, asChild: Platform.OS !== "web", children: _jsx(NativeOnlyAnimatedView, { className: "z-50", entering: FadeIn.reduceMotion(ReduceMotion.System), exiting: FadeOut.reduceMotion(ReduceMotion.System), as: "Pressable", children: _jsx(TextClassContext.Provider, { value: "text-popover-foreground", children: _jsxs(SelectPrimitive.Content, { ...props, className: cn("bg-popover border-border relative z-50 min-w-[8rem] rounded-md border shadow-md shadow-black/5", Platform.select({
                                ...
                                ...resolvedContentStyle,
                                        ...
                                        Platform.select({ web: "min-w-[var(--radix-select-trigger-width)]" }), !showScrollButtons && cn("max-h-[45vh] overflow-y-auto", "ui-menu-scrollbar"), viewportProps?.className), style: {
                                        ...(!showScrollButtons
                                            ? {
                                                maxHeight: windowHeight * SELECT_MENU_MAX_HEIGHT_RATIO,
                                                overflowY: "auto",
...
function SelectLabel({ className, ...props }) {
    return (_jsx(SelectPrimitive.Label, { className: cn("text-muted-foreground px-2 py-2 text-xs sm:py-1.5", className), ...props }));
}
function SelectItem({ className, children, description, startContent, endContent, itemIndicatorProps, itemTextProps, ...props }) {
    ...
    return (_jsxs(SelectPrimitive.Item, { ...props, label: label, className: cn("active:bg-accent group relative flex w-full flex-row items-center gap-2 rounded-sm py-2 pl-2 pr-8 sm:py-1.5", Platform.select({
            web: "focus:bg-accent focus:text-accent-foreground cursor-default outline-none data-[disabled]:pointer-events-none",
        }), props.disabled && "opacity-50", className), children: [_jsx(View, { className: "absolute right-2 flex size-3.5 items-center justify-center", children: _jsx(SelectPrimitive.ItemIndicator, { ...itemIndicatorProps, children: _jsx(Icon, { as: Check, className: "text-muted-foreground size-4 shrink-0" }) }) }), startContent, _jsxs(View, { className: "min-w-0 flex-1", children: [customLabel ? normalizeText(children) : null, _jsx(SelectPrimitive.ItemText, { ...itemTextProps, className: cn("text-foreground select-none text-sm group-active:text-accent-foreground", customLabel && "hidden", itemTextProps?.className) }), description != null ? normalizeText(description, "text-muted-foreground text-xs") : null] }), endContent] }));
}
...
function SelectScrollUpButton({ className, ...props }) {
    if (Platform.OS !== "web")
        return null;
    return (_jsx(SelectPrimitive.ScrollUpButton, { className: cn("flex cursor-default items-center justify-center py-1", className), ...props, children: _jsx(Icon, { as: ChevronUpIcon, className: "size-4" }) }));
}
function SelectScrollDownButton({ className, ...props }) {
    if (Platform.OS !== "web")
        return null;
    return (_jsx(SelectPrimitive.ScrollDownButton, { className: cn("flex cursor-default items-center justify-center py-1", className), ...props, children: _jsx(Icon, { as: ChevronDownIcon, className: "size-4" }) }));
}
