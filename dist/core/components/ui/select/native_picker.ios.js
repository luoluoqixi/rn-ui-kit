...
const NativePickerSwiftUIMenuTrigger = React.forwardRef(({ active, containerStyle, content, icon, keepPressedOpacity, label, labelProps, onPress, pressedOpacity, }, forwardedRef) => (_jsx(NativeTriggerPressable, { ref: forwardedRef, active: active, content: content, containerStyle: containerStyle, icon: icon, keepPressedOpacity: keepPressedOpacity, label: label, labelProps: labelProps, onPress: onPress, pressedOpacity: pressedOpacity })));
/** wheel + 原生 trigger（SwiftUI menu 按钮） */
const NativePickerWheelNativeTriggerSheet = React.forwardRef(({ nativeTriggerContainerStyle, nativeTriggerContent, nativeTriggerIcon, nativeTriggerLabel, nativeTriggerLabelProps, nativeTriggerPressedOpacity, items, placeholder, value, onValueChange, resolvedNativeHaptics, }, ref) => {
    const [pendingValue, setPendingValue] = React.useState(value ?? items[0]?.value ?? "");
    const [sheetName] = React.useState(() => `select-wheel-${++wheelSheetCounter}`);
    const openSheet = useCallback((shouldTriggerHaptics) => {
        if (shouldTriggerHaptics) {
    ...
    const title = typeof placeholder === "string" ? placeholder : "选择";
    return (_jsxs(_Fragment, { children: [_jsx(NativePickerSwiftUIMenuTrigger, { containerStyle: nativeTriggerContainerStyle, content: nativeTriggerContent, icon: nativeTriggerIcon, label: nativeTriggerLabel, labelProps: nativeTriggerLabelProps, onPress: () => openSheet(true), pressedOpacity: nativeTriggerPressedOpacity }), _jsx(WheelTrueSheet, { items: items, title: title, sheetName: sheetName, pendingValue: pendingValue, setPendingValue: setPendingValue, onCancel: handleCancel, onDone: handleDone })] }));
});
/**
 * dropdown + 自定义 trigger：复用 Menu 组件实现。
 * Menu 的 MenuTrigger 包装自定义 YStack，点击时显示选项列表。
 */
function NativePickerDropdownCustom({ items, value, placeholder, onValueChange, onOpenChange, open, resolvedNativeHaptics, nativeTrigger, nativeTriggerContainerStyle, nativeTriggerContent, nativeTriggerIcon, nativeTriggerLabel, nativeTriggerLabelProps, __menuRef, }) {
    const [internalOpen, setInternalOpen] = React.useState(false);
    const [willOpen, setWillOpen] = React.useState(null);
    const selectedLabel = items.find((item) => item.value === value)?.label ?? null;
    ...
    const handleOpenWillChange = useCallback((nextOpen) => {
        setWillOpen(nextOpen);
    }, []);
    const trigger = nativeTrigger ? (_jsx(NativePickerSwiftUIMenuTrigger, { active: isNativeTriggerActive, containerStyle: nativeTriggerContainerStyle, content: nativeTriggerContent, icon: nativeTriggerIcon, keepPressedOpacity: true, label: nativeTriggerLabel, labelProps: nativeTriggerLabelProps })) : (_jsx(NativePickerDefaultTrigger, { label: selectedLabel ?? (typeof placeholder === "string" ? placeholder : "选择"), placeholder: selectedLabel == null }));
    return (_jsx(Menu, { nativeHaptics: resolvedNativeHaptics, onOpenChange: handleOpenChange, onOpenWillChange: handleOpenWillChange, open: resolvedOpen, trigger: trigger, triggerProps: nativeTrigger ? { asChild: true } : undefined, 
...
export const NativePickerSwiftUI = React.forwardRef((props, ref) => {
    ...
    const { items, value, placeholder, mode, nativeTrigger, nativeTriggerContainerStyle, nativeTriggerContent, nativeTriggerIcon, nativeTriggerLabel, nativeTriggerLabelProps, nativeTriggerPressedOpacity, onValueChange, onOpenChange, resolvedNativeHaptics, } = props;
    // dropdown 组件
    if (mode === "dropdown") {
        return (_jsx(NativePickerDropdownCustom, { items: items, value: value, placeholder: placeholder, onValueChange: onValueChange, onOpenChange: (next) => {
                onOpenChange?.(next);
            }, resolvedNativeHaptics: resolvedNativeHaptics, nativeTrigger: nativeTrigger, nativeTriggerContainerStyle: nativeTriggerContainerStyle, nativeTriggerContent: nativeTriggerContent, nativeTriggerIcon: nativeTriggerIcon, nativeTriggerLabel: nativeTriggerLabel, nativeTriggerLabelProps: nativeTriggerLabelProps, __menuRef: menuControlRef }));
    }
