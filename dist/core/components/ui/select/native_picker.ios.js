...
const NativePickerSwiftUIMenuTrigger = React.forwardRef(({ containerStyle, content, icon, items, labelProps, value, onPress }, forwardedRef) => {
    const selectedValue = value ?? items[0]?.value ?? "";
    const selectedLabel = items.find((item) => item.value === selectedValue)?.label ?? "";
    return (_jsx(NativeTriggerPressable, { ref: forwardedRef, content: content, containerStyle: containerStyle, icon: icon, label: selectedLabel, labelProps: labelProps, onPress: onPress }));
});
...
function NativePickerDropdownCustom({ items, value, placeholder, onValueChange, onOpenChange, open, resolvedNativeHaptics, nativeTrigger, nativeTriggerContainerStyle, nativeTriggerContent, nativeTriggerIcon, nativeTriggerLabelProps, __menuRef, }) {
    ...
    const trigger = nativeTrigger ? (_jsx(NativePickerSwiftUIMenuTrigger, { containerStyle: nativeTriggerContainerStyle, content: nativeTriggerContent, icon: nativeTriggerIcon, items: items, labelProps: nativeTriggerLabelProps, value: value })) : (_jsx(NativePickerDefaultTrigger, { label: selectedLabel ?? (typeof placeholder === "string" ? placeholder : "选择"), placeholder: selectedLabel == null }));
    return (_jsx(Menu, { nativeHaptics: resolvedNativeHaptics, onOpenChange: handleOpenChange, open: resolvedOpen, trigger: trigger, triggerProps: nativeTrigger ? { asChild: true } : undefined, 
