...
export function ButtonNative({ androidColors, buttonSize, buttonColor, children, disabled: isDisabled, nativeComposeProps, onPress, style, title, variant, }) {
    ...
    const resolvedButtonColors = buttonColor == null
        ? overriddenColors ?? buttonColors
        : { ...(overriddenColors ?? buttonColors), contentColor: buttonColor };
    return (_jsx(Host, { matchContents: true, style: style, children: _jsx(Component, { ...composeButtonProps, colors: resolvedButtonColors, enabled: overriddenEnabled ?? !isDisabled, modifiers: overriddenModifiers ?? sizeModifiers, onClick: onPress, children: content }) }));
}
