...
export type SonnerNativeAdvancedOptions = Partial<Omit<SonnerNativeToastProps, "id" | "title" | "index" | "numberOfToasts" | "orderedToastIds">>;
...
export type ToastPromiseContent<ToastData = unknown> = RenderProp<ToastData | unknown> | ((data: ToastData | unknown) => Promise<ReactNode>);
...
export type ToastCustomFunc = (jsx: (id: string | number) => ReactElement, data?: ToastShowOptions) => string | number;
export type ToastPromiseFunc = <ToastData>(promise: ToastPromise<ToastData>, data?: ToastPromiseData<ToastData>) => {
