...
let toastId = 0;
const ToastDefaultNativeContext = React.createContext(undefined);
export function ToastDefaultsProvider({ children, defaultNative, }) {
    return (_jsx(ToastDefaultNativeContext.Provider, { value: defaultNative, children: children }));
}
function useNative(options, defaultNative) {
    if (isWeb())
        return false;
...
function show(title, kind, options, defaultNative) {
...
export function useToast() {
    ...
    const message = (title, options) => show(title, "default", resolveOptions(options), defaultNative);
    const info = (title, options) => show(title, "info", resolveOptions(options), defaultNative);
    const success = (title, options) => show(title, "success", resolveOptions(options), defaultNative);
    const error = (title, options) => show(title, "error", resolveOptions(options), defaultNative);
    const warning = (title, options) => show(title, "warning", resolveOptions(options), defaultNative);
    const loading = (title, options) => show(title, "loading", resolveOptions(options), defaultNative);
